<?php
	...
	//*********************************************************
	//trecho abaixo serve pra tratar o que vem em url amigavel e gerar parametros padrao antigo
	
	if ($_GET["busca"] != ""){
		die("<script> window.location.href='" . URL . "busca/" . @$_GET["busca"] . "'; </script>");
	}else if (@$_GET["origem"]=="busca"){
		$_GET["busca"] = $_GET["conteudo"];
	}else if (@$_GET["origem"]=="depar"){
		$_GET["depar"] = $_GET["conteudo"];
	}else if (@$_GET["origem"]=="tipo"){
		$_GET["depar"] = $_GET["conteudo"];
		$_GET["tipo"] = $_GET["conteudo2"];
	}else if (@$_GET["origem"]=="grupo"){
		$_GET["depar"] = $_GET["conteudo"];
		$_GET["tipo"] = $_GET["conteudo2"];
		$_GET["grupo"] = $_GET["conteudo3"];
		$_GET["codigo_grupo"] = $_GET["conteudo4"];
	}else if (@$_GET["origem"]=="categoria"){
		$_GET["depar"] = $_GET["conteudo"];
		$_GET["tipo"] = $_GET["conteudo2"];
		$_GET["grupo"] = $_GET["conteudo3"];
		 
		//padrao antigo onde via o codigo do grupo
		if (@$_GET["conteudo6"]!=""){
			$_GET["codigo_grupo"] = $_GET["conteudo4"];
			$_GET["categoria"] = $_GET["conteudo5"];
			$_GET["codigo_categoria"] = $_GET["conteudo6"];
		}else{
			$_GET["categoria"] = $_GET["conteudo4"];
			$_GET["codigo_categoria"] = $_GET["conteudo5"];
		}

		$_sqlCat = "SELECT CodigoGrupo FROM Categoria WHERE Codigo = '" . $_GET["codigo_categoria"] . "'";
		$_queryCat = mysql_query($_sqlCat);
		$_encontrado = mysql_num_rows($_queryCat);
	
	 	if ($_encontrado>0){
	 		while($_result = mysql_fetch_array($_queryCat))
	 			$_GET["codigo_grupo"] = $_result["CodigoGrupo"];
	 	}
	}

	//limpa as variáveis referentes ao SEO
	$_sqlSeo = "";
	$_tituloPagina = "";
	$_descricaoPagina = "";
	$_palavraChavePagina = "";
	...
	if(@$_GET["codigo"]!="")
		@$_GET["codigo_grupo"] = @$_GET["codigo"];
	
	//após implantação da Promoção implementar verificação para breadcrumb ficar com acento
	if (@$_GET["depar"]!="" || @$_GET["departamento"]!=""){
		
		if (@$_GET["departamento"]!="")
			@$_GET["depar"] = @$_GET["departamento"];
		
		$_texto = $_li;
		$_texto = str_replace("targetNome", mb_strtoupper(@$_GET["depar"],"UTF-8"), $_texto);
		$_texto = str_replace("targetLink", URL ."produtos/depar/" . removerAcento(@$_GET["depar"]), $_texto);
		
		$_breadcrumb[count($_breadcrumb)] = $_texto;
		$_tituloDescricao .= mb_strtoupper(@$_GET["depar"],"UTF-8");
	}
	...
	if (@$_GET["tipo"]!=""){
		...
		if(@$_GET["tipo"] == "Acessorio")
			$_tipo = "Acessório";
		else
			$_tipo = @$_GET["tipo"];

		$_texto = str_replace("targetNome", mb_strtoupper($_tipo,"UTF-8"), $_texto);
		
		if (@$_GET["depar"]!="")
			$_texto = str_replace("targetLink", URL ."produtos/tipo/" . removerAcento(@$_GET["depar"]) . "/" . @$_GET["tipo"], $_texto);
		else
			$_texto = str_replace("targetLink", URL ."produtos/tipo/" . removerAcento(@$_GET["tipo"]), $_texto);
		
		$_breadcrumb[count($_breadcrumb)] = $_texto;
		$_tituloDescricao .= " - " . mb_strtoupper(@$_GET["tipo"],"UTF-8");
	}
	
	if (@$_GET["grupo"]!=""){

		$_sqlSeo = "SELECT TituloPagina, DescricaoPagina, PalavraChavePagina, Nome";
		$_sqlSeo .= " FROM Grupo";
		$_sqlSeo .= " WHERE Codigo = '".@$_GET["codigo_grupo"]."'";
		
		$_querySeo = mysql_query($_sqlSeo);
		$_resultSeo = mysql_fetch_row($_querySeo);
		$_tituloPagina = $_resultSeo[0];
		$_descricaoPagina = $_resultSeo[1];
		$_palavraChavePagina = $_resultSeo[2];
		$_grupo = $_resultSeo[3];
		
		$_texto = $_li;
		$_texto = str_replace("targetNome", mb_strtoupper($_grupo,"UTF-8"), $_texto);
		
		$_link = URL ."produtos/grupo";
		$_link .= "/" . removerAcento(@$_GET["depar"]);
		$_link .= "/" . removerAcento(@$_GET["tipo"]);
		$_link .= "/" . removerAcento(@$_GET["grupo"]);
		$_link .= "/" . @$_GET["codigo_grupo"];
		
		$_texto = str_replace("targetLink", $_link, $_texto);	
		$_breadcrumb[count($_breadcrumb)] = $_texto;
		
		$_tituloDescricao .= " - " . mb_strtoupper(@$_GET["grupo"],"UTF-8");
	...
	if (@$_GET["categoria"]!=""){
		...
		$_sqlSeo = "SELECT TituloPagina, DescricaoPagina, PalavraChavePagina, Nome";
		$_sqlSeo .= " FROM Categoria";
		$_sqlSeo .= " WHERE Codigo = '".@$_GET["codigo_categoria"]."'";
		...
		$_descricaoPagina = $_resultSeo[1];
		$_palavraChavePagina = $_resultSeo[2];
		$_categoria = $_resultSeo[3]; 
		
		$_texto = $_li;
		$_texto = str_replace("targetNome", mb_strtoupper($_categoria,"UTF-8"), $_texto);
		
		$_link = URL ."produtos/categoria";
		$_link .= "/" . removerAcento(@$_GET["depar"]);
		$_link .= "/" . removerAcento(@$_GET["tipo"]);
		$_link .= "/" . removerAcento(@$_GET["grupo"]);
		$_link .= "/" . removerAcento(@$_GET["categoria"]);
		$_link .= "/" . @$_GET["codigo_categoria"];
		
		$_texto = str_replace("targetLink", $_link, $_texto);
		
		$_breadcrumb[count($_breadcrumb)] = $_texto;
		$_tituloDescricao .= " - " . mb_strtoupper(@$_GET["categoria"],"UTF-8");
	...
	if (@$_GET["busca"]!=""){
		...
		$_texto = $_li;
		$_texto = str_replace("targetNome", mb_strtoupper(@$_GET["busca"],"UTF-8"), $_texto);
		$_texto = str_replace("targetLink", URL ."produtos/busca/" . @$_GET["busca"], $_texto);
		
		$_breadcrumb[count($_breadcrumb)] = $_texto;
		$_tituloDescricao .= mb_strtoupper(@$_GET["busca"],"UTF-8");
	}
	
	if (count($_breadcrumb)==0){

		$_texto = $_li;
		$_texto = str_replace("targetNome", "PRODUTOS", $_texto);
		$_texto = str_replace("targetLink", URL ."produtos", $_texto);
		
		$_breadcrumb[count($_breadcrumb)] = $_texto;
		$_tituloDescricao .= "Produtos";
	}
...
<script>
	function buscar(pagPrm,ordPrm){
		...
		var data = {
					...
					<?php
						if (@$_GET["depar"]!="")
							echo "'depar':'" .  @$_GET["depar"] . "',";
						
						if (@$_GET["tipo"]!="")
							echo "'tipo':'" . @$_GET["tipo"] . "',";
						
						if (@$_GET["codigo_grupo"]!=""){
							echo "'codigo_grupo':'" . @$_GET["codigo_grupo"] . "',";
							echo "'grupo':'" . @$_GET["grupo"]. "',";
						}
						
						if (@$_GET["codigo_categoria"]!=""){
							echo "'codigo_categoria':'" . @$_GET["codigo_categoria"] . "',";
		...
		$.ajax({
	        url:'<?php echo URL; ?>includes/listagemProdutos.php',
	        type:'POST',
	        data: data,
	        success: function(result){
		       	if (pagina=="1"){
		        	$('#my_section').html(result);
		        	$("html,body").animate({scrollTop: $(".conteudo .limites").offset().top},'slow');
		        }else
		        	$('#my_section').append(result);
	    	}
	 	});
	}
