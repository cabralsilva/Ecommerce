...
$contents = ftp_nlist($conexao_ftp, "/$pasta_ftp/$sub_pasta_ftp/");	
$total_arquivos_pasta = count($contents);

$remove  = $_REQUEST['remove'];

$file = "/$pasta_ftp/$sub_pasta_ftp/$remove";
...
if (ftp_delete($conexao_ftp, $file)) {
   ...
   echo "não foi possível excluir $file\n";
	...
	for ($_i=1; $_i<=$total_arquivos_pasta; $_i++){
		$old_file = "/$pasta_ftp/$sub_pasta_ftp/$nome_inicial_ftp".$_i.".jpg";  
		//$j = $_i+1;
		$new_file = "/$pasta_ftp/$sub_pasta_ftp/$tmp_w_".$_i.".jpg";  
	...
	for($x=0; $x<$totala; $x++){
		$nome_tmp = $contents[$x];
		//$_i = $x + 1;
		$old_file = "/$pasta_ftp/$sub_pasta_ftp/".$nome_tmp;  
		//$j = $_i+1;
		$new_file = "/$pasta_ftp/$sub_pasta_ftp/$nome_inicial_ftp".$x.".jpg";  
		@ftp_rename($conexao_ftp, $old_file, $new_file);
	}
}  else{
 
//tenta excluir $file
if (ftp_delete($conexao_ftp,  $_SESSION["s_pasta_ftp"]."/".$_SESSION["s_sub_pasta_ftp"]."/".$_REQUEST["remove"])) {
