<?php
	...
	include_once("analise/tagEvento.php");
	
	verificaSSL(false);
	
	$_tituloPagina = "Fale conosco";
	
	$_data = date("d/m/Y");
	$_hora = date('h:i:s A');
	
	if (!validaEmail($_POST["email"])){
		echo "<script> alert('E-mail invalido!'); window.location.href='" . URL . "faleConosco'; </script>";
		die;
	}
	
	$destinatarios = 'plander@plander.com.br';
	$nomeDestinatario = 'plander@plander.com.br';
	
	$usuario = 'plander@plander.com.br';
	$senha = 'ZION16equip!';
	$nomeRemetente = "Plander";
	$assunto = "Fale conosco";
	$_conteudo = "<strong><u>Mensagem da p&aacute;gina Contato</u></strong><br><br><br>";
	
	$_conteudo .= "Nome: <br><strong>" . $_POST['nome'] . "</strong><br><br>";
	$_conteudo .= "E-mail: <br><strong>" . $_POST['email'] . "</strong><br><br>";
	$_conteudo .= "Assunto: <br><strong>" . $_POST['assunto']. "</strong><br><br><br>";
	$_conteudo .= "Mensagem: <br><strong>" . $_POST['mensagem'] . "</strong><br><br><br>";
	$_conteudo .= "Enviado dia " . $_data . " &agrave;s " . $_hora . ".</strong>";
	/*	
	...
	//$mail->AddCC("plander@plander.com.br","Plander");
...
<div class="conteudo" role="main">
  ...
  <div class="limites">
    ...
    <ol class="breadcrumb" itemscope itemtype="http://schema.org/BreadcrumbList">
      ...
      <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
        <a itemprop="item" href="<?php echo URL; ?>faleConosco"><span itemprop="name">Fale Conosco</span></a>
    ...
    <h2>Fale Conosco</h2>
	<?php
		//NO HOMOLOGAÇÃO TEM QUE SER ENVIADO AO suporte@iboltsys.com.br
		//NO PRODUÇÃO PARA $destinatarios
		$sendEmail = sendEmail("suporte@iboltsys.com.br", $assunto, $_conteudo);
		if ($sendEmail != 1){
			...
			echo "<h3>Ocorreu algum erro durante o envio da mensagem! Tente novamente mais tarde!</h3>";
    		
    		echo "<a href=\"" . URL . "faleConosco\" class=\"botao voltar\"><svg width=\"8px\" height=\"15px\"><use xlink:href=\"#icone-seta\" class=\"icone\" /></svg> Enviar novamente</a>";
		}else{
			echo "<h3>Sua mensagem foi enviada com sucesso!</h3>";
    		echo "<p>Obrigado por entrar em contato com a Plander, entraremos em contato o mais breve possível.</p>";
    		echo "<a href=\"" . URL . "index\" class=\"botao voltar\"><svg width=\"8px\" height=\"15px\"><use xlink:href=\"#icone-seta\" class=\"icone\" /></svg> Voltar à página inicial</a>";
		}
	?>
  </div>
  <!-- fecha .limites -->
