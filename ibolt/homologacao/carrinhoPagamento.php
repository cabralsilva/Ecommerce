<?php
	...
	$_tituloPagina = "Pagamento - Carrinho de Compras";
	$_carrinho = true;
	
	//if (@$_SESSION["PEDIDO"]["USUARIO"]=="" && @$_SESSION["PEDIDO"]["email"]==""){
    ...
    <div class="limites">
      ...
      <div class="resumo-pedido">
        <div class="esquerda">
          ...
          <?php
          		echo "<h4>" . $_SESSION["ENTREGA"]["nome"] . "</h4>";
				echo "<p>";
				echo $_SESSION["ENTREGA"]["endereco"];
				echo $_SESSION["ENTREGA"]["numero"]!=""?", " . $_SESSION["ENTREGA"]["numero"]:"";
				echo $_SESSION["ENTREGA"]["complemento"]!=""?" - " . $_SESSION["ENTREGA"]["complemento"]:"";
				echo "</p>";
				echo "<p>";
				echo $_SESSION["ENTREGA"]["cidade"] . " - " . $_SESSION["ENTREGA"]["uf"] . " - CEP " . $_SESSION["ENTREGA"]["cep_completo_formatado"];
				echo "</p>";
          ?>
          <a href="<?php echo URL_SSL; ?>carrinhoEnderecoEntrega" class="botao" title="Alterar endereço de entrega">Alterar endereço de entrega <svg width="8px" height="15px"><use xlink:href="#icone-seta" class="icone" /></svg></a>
        </div>
        <div class="direita">
          <h3>Meu cadastro</h3>
          <?php
          		echo "<h4>" . $_SESSION[@$_SESSION["PEDIDO"]["USUARIO"]]["nome_principal"] . "</h4>";
				echo "<p>";
				echo $_SESSION[@$_SESSION["PEDIDO"]["USUARIO"]]["endereco"];
				echo $_SESSION[@$_SESSION["PEDIDO"]["USUARIO"]]["numero"]!=""?", " . $_SESSION[@$_SESSION["PEDIDO"]["USUARIO"]]["numero"]:"";
				echo $_SESSION[@$_SESSION["PEDIDO"]["USUARIO"]]["complemento"]!=""?" - " . $_SESSION[@$_SESSION["PEDIDO"]["USUARIO"]]["complemento"]:"";
				echo "</p>";
				echo "<p>";
				echo $_SESSION[@$_SESSION["PEDIDO"]["USUARIO"]]["cidade"] . " - " . $_SESSION[@$_SESSION["PEDIDO"]["USUARIO"]]["uf"] . " - CEP " . $_SESSION[@$_SESSION["PEDIDO"]["USUARIO"]]["cep_completo_formatado"];
				echo "</p>";
          ?>
          <a href="<?php echo URL_SSL; ?>carrinhoCadastro/editar" class="botao" title="Alterar meu cadastro">Alterar cadastro <svg width="8px" height="15px"><use xlink:href="#icone-seta" class="icone" /></svg></a>
          <!--<a href="carrinho-seus-dados.html" class="botao" title="Alterar meu cadastro">Alterar senha <svg width="8px" height="15px"><use xlink:href="#icone-seta" class="icone" /></svg></a>-->
        </div>
        <div class="pedido">
          <h3>Meu pedido</h3>
          <table class="compras tablesaw tablesaw-stack" data-tablesaw-mode="stack">
            ...
            <?php
				...
				if ($_transportadora==false){
					...
					$_frete = calcularFrete($_SESSION["ENTREGA"]["cep_completo"], $_peso, $_altura, $_largura, $_comprimento);
					
					if($_frete!="n"){
						//$_valorPac = str_replace('.',',',$_frete[0]);
						//$_valorSedex = str_replace('.',',',$_frete[1]);
						$_valorPac = $_frete[0];
						$_valorSedex = $_frete[1];
						$_prazoPac = str_replace('.',',',$_frete[2]);
						$_prazoSedex = str_replace('.',',',$_frete[3]);
						
						if (@$_GET["tipo_frete"]=="0" || @$_GET["tipo_frete"]=="")
							$_SESSION["PEDIDO"]["valor_frete"] = $_valorPac;
						else if (@$_GET["tipo_frete"]=="1")
							$_SESSION["PEDIDO"]["valor_frete"] = $_valorSedex;
          ...
          <a href="<?php echo URL; ?>carrinho" class="voltar" title="Editar carrinho"><svg width="8px" height="15px"><use xlink:href="#icone-seta" class="icone" /></svg> Editar carrinho</a>
        </div>
      </div>

      <form action="<?php echo URL_SSL; ?>actions/finalizarPedido.php" method="POST" onsubmit="return mySubmit()">
        <fieldset class="opcoes-entrega">
          <h3>Opções de entrega</h3>
          <ul>
          	<?php
          		$_SESSION["PEDIDO"]["transportadora"] = false;
				$_SESSION["PEDIDO"]["valor_frete"] = 0;
				
				$_SESSION["PEDIDO"]["fretes_disponiveis"] = array();
				$_SESSION["PEDIDO"]["fretes_disponiveis"]["2"] = 0; //transportadora
				$_SESSION["PEDIDO"]["fretes_disponiveis"]["3"] = 0; //retirar na loja
				
          		if ($_transportadora==false && $_erroFrete==false){
          			echo "<li><label><input type=\"radio\" name=\"entrega\" value=\"0\" checked=\"checked\">";
          			echo "<img src=\"" . URL_SSL . "imagens/carrinho-pac.png\" alt=\"PAC\">";
					echo $_arrayFrete["prazo_pac"] . " dias úteis <span>R$ " . number_format($_arrayFrete["valor_pac"],2,",",".") . "</span>";
          			echo "</label></li>";
					
					echo "<li><label><input type=\"radio\" name=\"entrega\" value=\"1\">";
          			echo "<img src=\"" . URL_SSL . "imagens/carrinho-sedex.png\" alt=\"SEDEX\">";
					echo $_arrayFrete["prazo_sedex"] . " dias úteis <span>R$ " . number_format($_arrayFrete["valor_sedex"],2,",",".") . "</span>";
          			echo "</label></li>";
					
					if (@$_GET["tipo_frete"]=="1")
						$_SESSION["PEDIDO"]["valor_frete"] = $_arrayFrete["valor_sedex"];
					else
						$_SESSION["PEDIDO"]["valor_frete"] = $_arrayFrete["valor_pac"];
					
					$_SESSION["PEDIDO"]["fretes_disponiveis"]["0"] = $_arrayFrete["valor_pac"];
					$_SESSION["PEDIDO"]["fretes_disponiveis"]["1"] = $_arrayFrete["valor_sedex"];
				}else if ($_transportadora){
          			$_SESSION["PEDIDO"]["transportadora"] = true;
					
					$_SESSION["PEDIDO"]["fretes_disponiveis"]["2"] = 0;
          		}else if ($_erroFrete){
          			echo "<li><label>";
					echo "Ocorreu algum erro durante o cálculo do frete";
					echo "</label></li>";
          		}
				
				echo "<li><label><input type=\"radio\" name=\"entrega\" value=\"3\">";
				echo "<img src=\"" . URL_SSL . "imagens/carrinho-retirada.png\" alt=\"Retirada na loja\">";
				
				if ($_transportadora)
          			echo "<li><label><input type=\"radio\" name=\"entrega\" value=\"2\"" . ($_transportadora?"checked=\"checked\"":"") . "><img src=\"" . URL_SSL . "imagens/carrinho-transportadora.png\" alt=\"Transportadora\"></label></li>";
				
      			echo "</label></li>";
			?>
            <!--<li><label><input type="radio" name="entrega" value="3" ><img src="imagens/carrinho-retirada.png" alt="Retirada na loja"></label></li>
            <li><label><input type="radio" name="entrega"><img src="imagens/carrinho-pac.png" alt="PAC">5 a 15 dias úteis <span>R$3,99</span></label></li>
            <li><label><input type="radio" name="entrega"><img src="imagens/carrinho-sedex.png" alt="SEDEX">Até 5 dias úteis <span>R$15,99</span></label></li>
            <li><label><input type="radio" name="entrega"><img src="imagens/carrinho-e-sedex.png" alt="E-SEDEX">Até 5 dias úteis <span>R$15,99</span></label></li>
            <li><label><input type="radio" name="entrega"><img src="imagens/carrinho-transportadora.png" alt="Transportadora"></label></li>-->
          </ul>
        </fieldset>
        <fieldset class="formas">
          <h3>Formas de pagamento</h3>
          <ul>
          	<li><label><input type="radio" id="radio_visa" name="pagamento" value="0" checked="checked"><img src="<?php echo URL_SSL; ?>imagens/pagamento-visa.png" alt="Visa"></label></li>
            <li><label><input type="radio" id="radio_master" name="pagamento" value="1"><img src="<?php echo URL_SSL; ?>imagens/pagamento-master.png" alt="MasterCard"></label></li>
            <li><label><input type="radio" id="radio_boleto" name="pagamento" value="2"><img src="<?php echo URL_SSL; ?>imagens/pagamento-boleto.png" alt="Boleto"></label></li>
          </ul>
        </fieldset>
        <fieldset class="pagamento">
          <ol>
            <li class="obrigatorio">
              <ul>
                <li><label for="numero-cartao">Número do cartão  &nbsp;&nbsp;&nbsp;(somente números)</label><input class="grande" type="text" name="numero-cartao" id="numero-cartao" maxlength="16"></li>
                <li><label for="codigo-cartao">Código de segurança</label><input class="pequeno" type="text" name="codigo-cartao" id="codigo-cartao" maxlength="3"></li>
              </ul>
            </li>
            <!--<li class="obrigatorio"><label for="documento">CPF / CNPJ do titular do cartão</label><input class="grande" type="text" name="documento" id="documento"></li>-->
            <li class="obrigatorio"><label for="nome-impresso">Nome impresso no cartão</label><input class="grande" type="text" name="nome-impresso" id="nome-impresso"></li>
            <li class="obrigatorio">
...
<script>
	...
	$('input[name=entrega]').click(function(){
		//if ($(this).val()!="2"){ //nao fazer nd quando transportadora
			var dados = {"tipo":$(this).val()};
			$.ajax({
				url: "<?php echo URL_SSL; ?>actions/alterarFretePagamento.php",
				data: dados,
