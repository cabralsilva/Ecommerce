<?php
	...
	$_tituloPagina = "Confirmação - Carrinho de Compras";
	$_carrinho = true;
	
	if (is_array(@$_SESSION['CART'])==false)
		die("<script> window.location.href='" . URL_SSL . "index'; </script>");
?>
<!DOCTYPE html>
...
<div class="carrinho">
  ...
  <div class="conteudo">
    ...
    <div class="limites">
      ...
      <div class="confirmacao">
        <h3>Obrigado por comprar na Plander!</h3>
        <h4>Número do pedido: <span><?php echo @$_SESSION["PEDIDO"]["codigo"]; ?></span></h4>
        <p>Você receberá um e-mail em breve com a confirmação do pedido no endereço <?php echo @$_SESSION["PEDIDO"]["email"]; ?>.</p>
        
        <p><a href="javascript:void(0);" onclick="window.open('<?php echo URL_SSL; ?>actions/pedidoComprovante.php?id=<?php echo $_SESSION["PEDIDO"]["codigo"]; ?>');" title="Imprimir comprovante da compra">Imprimir comprovante da compra</a></p>
        
        <?php
        	if (@$_SESSION["PEDIDO"]["forma_pagamento"]=="boleto")
				echo "<a href=\"javascript:void(0);\" onclick=\"window.open('boleto/boleto_bradesco.php?id=" . $_SESSION["PEDIDO"]["codigo"] . "','janelaBloq', 'width=800, height=650, top=0, left=0, scrollbars=no, status=no, resizable=no, directories=no, location=no, menubar=no, titlebar=no, toolbar=no');\" class=\"botao action menor\" title=\"Imprimir boleto para pagamento\">Imprimir boleto <svg width=\"8px\" height=\"15px\"><use xlink:href=\"#icone-seta\" class=\"icone\" /></svg></a>";
        ?>
        <div class="duvidas">
          <h5>Dúvidas?</h5>
          <p>Ligue para <?php echo TELEFONE; ?> ou entre em <a href="<?php echo URL_SSL; ?>faleConosco" title="Entre em contato com a Plander">contato</a> conosco.</p>
        </div>
      </div>
        ...
        <table>
          <tfoot>
            <tr>
              ...
              <td>R$ <?php echo number_format($_SESSION["PEDIDO"]["subtotal"],2,",","."); ?></td>
            </tr>
            <tr>
              <th scope="row">Frete</th>
              <td>R$ <?php echo number_format($_SESSION["PEDIDO"]["valor_frete"],2,",","."); ?></td>
            </tr>
            <tr>
              <th scope="row">Total</th>
              <td>R$ <?php echo number_format($_SESSION["PEDIDO"]["subtotal"]+$_SESSION["PEDIDO"]["valor_frete"],2,",","."); ?></td>
            </tr>
          </tfoot>
        </table>
        <a href="<?php echo URL_SSL; ?>" class="botao voltar" title="Voltar para a página inicial"><svg width="8px" height="15px"><use xlink:href="#icone-seta" class="icone" /></svg> Voltar para o site</a>
        <!--<a href="carrinho" class="botao" title="Fazer um novo pedido">Fazer novo pedido</a>-->
      </div>
  ...
  <div id="modal-loading" class="modal">
    ...
    <div class="modal-content">
      ...
      <p>Enviando email do comprovante de compra...</p>
    </div>
...
<script src="<?php echo URL_SSL; ?>js/plugins.min.js"></script>
<script src="<?php echo URL_SSL; ?>js/_bower.min.js"></script>
<script src="<?php echo URL_SSL; ?>js/main.min.js"></script>
<!-- fecha scripts -->
<script type="text/javascript">
  $(document).ready(function(){
    var modal = document.getElementById('modal-loading');
    $.ajax({
      url: 'actions/pedidoComprovante.php',
      type: 'GET',
      data: 'email=1',
      async: false,
      beforeSend: function() {
      ...
      error: function(e) {
        alert("Seu pedido foi finalizado com sucesso, porém houve uma falha ao enviar o email com comprovante de compra. Você pode gerar o comprovante clicando no link \"Imprimir comprovante da compra\". Caso a falha persista entre em contato com a plander para solicitá-lo no Telefone (41) 3323-3636");
