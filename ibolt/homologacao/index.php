...
<div class="conteudo" role="main">
  ...
  <section class="limites">
    <div class="marcas swiper-container">
      <div class="swiper-wrapper">
      	<?php
			/*@$_filesFabricantes = glob("banners/fabricantes/*.jpg");
			$_totalFabricantes = strlen(@$_filesFabricantes[0]);
			
			$_arqFabricantes = array();
			if ($_totalFabricantes>0){		
				//o nome do arquivo vira o key do vetor
				for ($_i=0; $_i < count($_filesFabricantes); $_i++) 
					$_arqFabricantes[end(explode("/", $_filesFabricantes[$_i]))] = $_filesFabricantes[$_i];
				
				ksort($_arqFabricantes);
			}
			
			$_keys = array_keys($_arqFabricantes);		
      		for($_i=0; $_i < count(@$_keys); $_i++) {
				$_link = $_keys[$_i];
				$_link   = decodeNameImageUrl($_link);
				$_link   = str_replace(".jpg","",$_link);
				...
				echo "<div class=\"swiper-slide\">";
				echo "<a href=\"" . $_link . "\" title=\"Ver produtos deste fabricante\">";
				echo "<img src=\"" . $_arqFabricantes[$_keys[$_i]] . "\" alt=\"Destaque\">";
				echo "</a>";
				echo "</div>";
