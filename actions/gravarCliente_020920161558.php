<?php
	...
	if (@$_GET["tabela"]==TB_CLIENTE){
		...
		if ($_SESSION["PEDIDO"]["USUARIO"]=="NOVO"){
			...
			$_pedido['ClienteCep'] = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["cep_completo"];
			...
			if (isset($_SESSION["PEDIDO"]['codigo'])) {
				...
				if ($resposta->codStatus != 1) {
					echo "<script> alert('Ops, houve um erro na comunicação com o banco de dados (ACN). Tente novamente!'); </script>";
			...
			$_cliente['Cep'] = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["cep_completo"];
			...
			if (@$_POST["alterar_entrega"]=="1"){
				...
				if ($resposta->codStatus != 1) {
					echo "<script> alert('Ops, houve um erro na comunicação com o banco de dados. Tente novamente (ACNEE)!'); </script>";
					echo "<script> parent.window.location.href='../carrinhoCadastro'; </script>";
					die();
				}
			}else {
				...
				if ($resposta->codStatus != 1) {
					echo "<script> alert('Ops, houve um erro na comunicação com o banco de dados(ACV). Tente novamente!'); </script>";
					echo "<script> parent.window.location.href='../carrinhoCadastro'; </script>";
					die();
				}
			}
