<?php
	...
	$_tituloPagina = "Cadastro - Carrinho de Compras";
	$_carrinho = true;
	
	//*************************************************************************************************************
	//alimento variaveis vazias que podem ser preenchidas e inseridas nos inputs caso seja edição do cadastro novo
	
	//---------------------------------------------------
	$_loginAuto = false; //facilitar desenvolvimento
	//---------------------------------------------------
	...
	if ($_loginAuto){
		$_nome = "Jozias Machado";
		$_dataNascimento = "1988-09-03";
		$_cpf = "06849131940";
		$_rg = "10396";
		...
		$_cep1 = "83602";
		$_cep2 = "360";
		$_endereco = "Domingos cordeiro";
		$_numero = "2342";
		$_complemento = "casa123";
		$_bairro = "são vicente";
		$_uf = "PR";
		$_cidade = "campo largo";
		
		$_informacoesReferencia = "igreja aparecida";
	}
	
	//*************************************************************************************************************
	
	if (@$_SESSION["PEDIDO"]["USUARIO"]!=""){
		$_pessoa = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["pessoa"];
		
		$_nome = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["nome"];
		$_dataNascimento = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["data_nascimento"];
		$_cpf = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["cpf"];
		$_rg = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["rg"];
		
		$_razaoSocial = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["razao_social"];
		$_cnpj = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["cnpj"];
		$_ie = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["ie"];
		
		$_email = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["email"];
		
		$_ddd1 = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["ddd1"];
		$_telefone1 = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["telefone1"];
		$_ddd2 = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["ddd2"];
		$_telefone2 = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["telefone2"];
		
		$_cep1 = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["cep1"];
		$_cep2 = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["cep2"];
		
		$_endereco = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["endereco"];
		$_numero = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["numero"];
		$_complemento = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["complemento"];
		$_bairro = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["bairro"];
		$_uf = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["uf"];
		$_cidade = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["cidade"];
		
		$_informacoesReferencia = $_SESSION[$_SESSION["PEDIDO"]["USUARIO"]]["informacoes_referencia"];
	}else if (@$_SESSION["email"]!="" && @$_GET["acao"]==""){
		$_email = $_SESSION["email"];
	}else
		die("<script> window.location.href='" . URL_SSL . "carrinhoLogin'; </script>");
?>
<!DOCTYPE html>
...
<div class="carrinho">
  ...
  <div class="conteudo">
    ...
    <div class="limites">
      ...
      <p>Para ser cliente (pessoa Física ou Jurídica) é necessário preencher corretamente o formulário abaixo com os respectivos dados cadastrais.</p>
      <p>Os campos em NEGRITO são de preenchimento obrigatório e essenciais para processarmos o envio do seu futuro pedido.</p>

      <ul class="abas">
        <li><a id="abas_fisica" href="javascript:void(0);" title="Dados de pessoa física">Pessoa física</a></li>
        <li><a id="abas_juridica" href="javascript:void(0);" title="Dados de pessoa jurídica">Pessoa jurídica</a></li>
      </ul>

      <form action= "<?php echo URL_SSL; ?>actions/gravarCliente.php?tabela=<?php echo TB_CLIENTE; ?>&acao=<?php echo @$_GET["acao"]!=""?@$_GET["acao"]:"novo"; ?>" method="POST" onsubmit="return mySubmit();" class="conta">
        ...
        <fieldset>
          <section id="section_fisica">
            ...
            <ol>
              ...
              <li class="obrigatorio">
              	...
              	<input class="pequeno" type="date" name="data-nascimento" id="data-nascimento" placeholder="dd/mm/aaaa" value="<?php echo $_dataNascimento; ?>">
              </li>
              <li>
                <ul>
                  <li class="obrigatorio"><label for="cpf">CPF</label><input class="medio" type="text" name="cpf" id="cpf" value="<?php echo $_cpf; ?>"></li>
                  <li class="obrigatorio"><label for="rg">RG</label><input class="medio" type="text" name="rg" id="rg" value="<?php echo $_rg; ?>"></li>
                </ul>
              </li>
              <li class="obrigatorio"><label for="email">Endereço de e-mail</label><input class="medio" type="email" name="email-fisica" id="email-fisica" value="<?php echo $_email; ?>"> <small>(será usado como usuário)</small></li>
              <li>
                <ul>
                  <li class="obrigatorio"><label for="ddd">DDD</label><input class="medio" type="tel" name="ddd-fisica" id="ddd-fisica" value="<?php echo $_ddd1; ?>"></li>
                  <li class="obrigatorio"><label for="telefone">Telefone</label><input class="medio" type="tel" name="telefone-fisica" id="telefone-fisica" value="<?php echo $_telefone1; ?>"></li>
                </ul>
              </li>
              <li>
                <ul>
                  <li><label for="ddd2">DDD</label><input class="medio" type="tel" name="ddd2-fisica" id="ddd2-fisica" value="<?php echo $_ddd2; ?>"></li>
                  <li><label for="telefone2">Outro telefone</label><input class="medio" type="tel" name="telefone2-fisica" id="telefone2-fisica" value="<?php echo $_telefone2; ?>"></li>
                </ul>
              </li>
            </ol>
          </section>
          ...
          <section id="section_juridica">
            ...
            <ol>
              ...
              <li>
                <ul>
                  <li class="obrigatorio"><label for="cnpj">CNPJ</label><input class="medio" type="text" name="cnpj" id="cnpj" value="<?php echo $_cnpj; ?>"></li>
                  <li class="obrigatorio"><label for="ie">Inscrição Estadual</label><input class="medio" type="text" name="ie" id="ie" value="<?php echo $_ie; ?>"></li>
                </ul>
              </li>
              <li class="obrigatorio"><label for="email">Endereço de e-mail</label><input class="medio" type="email-juridica" name="email-juridica" id="email" value="<?php echo $_email; ?>"> <small>(será usado como usuário)</small></li>
              <li>
                <ul>
                  <li class="obrigatorio"><label for="ddd">DDD</label><input class="medio" type="tel" name="ddd-juridica" id="ddd-juridica" value="<?php echo $_ddd1; ?>"></li>
                  <li class="obrigatorio"><label for="telefone">Telefone</label><input class="medio" type="tel" name="telefone-juridica" id="telefone-juridica" value="<?php echo $_telefone1; ?>"></li>
                </ul>
              </li>
              <li>
                <ul>
                  <li><label for="ddd2">DDD</label><input class="medio" type="tel" name="ddd2-juridica" id="ddd2-juridica" value="<?php echo $_ddd2; ?>"></li>
                  <li><label for="telefone2">Outro telefone</label><input class="medio" type="tel" name="telefone2-juridica" id="telefone2-juridica" value="<?php echo $_telefone2; ?>"></li>
                </ul>
              </li>
            </ol>
		  </section>
          ...
          <section>
            ...
            <ol>
              <li class="obrigatorio">
                ...
                <ul>
                  <li><input class="cep" type="text" name="cep1" id="cep1" title="Digite o CEP" maxlength="5" value="<?php echo $_cep1; ?>"></li>
                  <li><input class="cep-menor" type="text" name="cep2" id="cep2" title="Digite o CEP" maxlength="3" value="<?php echo $_cep2; ?>"></li>
                </ul>
              </li>
              <li>
                <ul>
                  <li class="obrigatorio"><label for="endereco">Endereço</label><input class="grande" type="text" name="endereco" id="endereco" value="<?php echo $_endereco; ?>"></li>
                  <li class="obrigatorio"><label for="endereco-numero">Número</label><input class="cep" type="text" name="endereco-numero" id="endereco-numero" value="<?php echo $_numero; ?>"></li>
                </ul>
              </li>
              <li>
                <ul>
                  <li><label for="complemento">Complemento</label><input class="medio" type="text" name="complemento" id="complemento" value="<?php echo $_complemento; ?>"></li>
                  <li class="obrigatorio"><label for="bairro">Bairro</label><input class="medio" type="text" name="bairro" id="bairro" value="<?php echo $_bairro; ?>"></li>
                </ul>
              </li>
              <li>
                <ul>
                      ...
                      <select name="estado" id="estado">
                        <?php
                        	for ($_i=0; $_i < count($_arrayUf); $_i++) { 
								echo "<option value=\"" . $_arrayUf[$_i] . "\" " . (($_uf==$_arrayUf[$_i]||($_uf=="" && $_arrayUf[$_i]=="PR"))?"selected=\"selected\"":"") . ">" . $_arrayUf[$_i] . "</option>";
							}
                        ?>
                      </select>
                    </div>
                  </li>
                  <li class="obrigatorio"><label for="cidade">Cidade</label><input class="medio" type="text" name="cidade" id="cidade" value="<?php echo $_cidade; ?>"></li>
                </ul>
              </li>
              <li><label for="pontos">Pontos de referência</label><textarea name="pontos" id="pontos" cols="30" rows="10"><?php echo $_informacoesReferencia; ?></textarea></li>
              
              <li class="li_entrega">
              	<label for="alterar_entrega">Entrega</label>
              	<div class="select medio">
	              	<select name="alterar_entrega" id="alterar_entrega"> 
						<option value="1" selected="selected">Alterar endereço de entrega</option>
						<option value="0">Não alterar endereço de entrega</option>
	                  </select>
	             </div>
              </li>
          ...
          <section class="section_senha">
            <h3>Senha</h3>
            <ol>
              <li class="obrigatorio">
                <ul>
                  <li class="obrigatorio"><label for="senha">Nova senha</label><input class="pwd" type="password" name="senha" id="senha"></li>
   			      <li class="obrigatorio"><label for="confirmacao">Confirmar nova senha</label><input class="pwd" type="password" name="confirmacao" id="confirmacao"></li>
                </ul>
              </li>
            </ol>
          </section>
          <a href="<?php echo URL_SSL . (@$_GET["acao"]=="editar"?"carrinhoPagamento":"carrinhoLogin/1"); ?>" class="voltar" title="Voltar à página anterior"><svg width="8px" height="15px"><use xlink:href="#icone-seta" class="icone" /></svg> Voltar</a>
          <button type="submit">Confirmar <svg width="8px" height="15px"><use xlink:href="#icone-seta" class="icone" /></svg></button>
        </fieldset>
      </form>
      
	  <div class="alterar-senha">
        <h3>Alterar senha</h3>
        <a href="<?php echo URL_SSL; ?>carrinhoSenha" class="botao" title="Alterar senha">Alterar senha <svg width="8px" height="15px"><use xlink:href="#icone-seta" class="icone" /></svg></a>
      </div>
...
<script src="<?php echo URL_SSL; ?>js/jquery-maskinput-1.1.4.js" defer></script>

<script>
  $(document).ready(function(){
    $("#cpf").mask("999.999.999-99");
    $("#cnpj").mask("99.999.999/9999-99");
		var selecionarAba = "<?php echo $_pessoa; ?>";
		
		$("#abas_fisica").click(function(){
			$("#section_juridica").slideUp();
			
			if ($("#abas_juridica").parent().hasClass("ativo"))
				$("#abas_juridica").parent().removeClass("ativo");
			
			$("#section_fisica").slideDown();
			$("#abas_fisica").parent().addClass("ativo");
			
			$("#pessoa").val("fisica");
		});
		
		$("#abas_juridica").click(function(){
			$("#section_fisica").slideUp();
			
			if ($("#abas_fisica").parent().hasClass("ativo"))
				$("#abas_fisica").parent().removeClass("ativo");
			
			$("#section_juridica").slideDown();
			$("#abas_juridica").parent().addClass("ativo");
			
			$("#pessoa").val("juridica");
		});
		
		$("#abas_" + selecionarAba).click();
		
		<?php
			if (strtoupper(@$_SESSION["PEDIDO"]["USUARIO"])=="CADASTRADO"){
				echo "$(\".abas\").hide();";
				echo "$(\".section_senha\").hide();";
				echo "$(\"#email-fisica\").prop('disabled',true);";
				echo "$(\"#email-juridica\").prop('disabled',true);";
			}else{
				echo "$(\".alterar-senha\").hide();";
			}
			if (strtoupper(@$_SESSION["PEDIDO"]["USUARIO"])=="")
				echo "$(\".li_entrega\").hide();";
		?>
	    ...
	    $("#cep2").keyup(function(e) {
			...
			if (cepCampo1.length == 5 && $("#cep2").val().length == 3) {
				...
				$.ajax({
					url: "<?php echo URL_SSL; ?>actions/inputCepXml.php",
					data: dados,
	...
	function mySubmit(){
    //INICIO VALIDAÇÃO CPF/CNPJ
    var cpf_cnpj = $("#pessoa").val() == "fisica" ? $('input[name=cpf]').val() : $('input[name=cnpj]').val();
    var pai = $("#pessoa").val() == "fisica" ? $('input[name=cpf]').parent() : $('input[name=cnpj]').parent();
    if (pai.hasClass("erro")) pai.removeClass("erro");
    var returnCpfCnpj = 1;
    $.ajax({
      url: "<?php echo URL_SSL; ?>actions/validaCpfCnpj.php",
      type: 'POST',
      async: false,
      data: {
      ...
      error: function(e) {
        ...
        pai.addClass("erro");
        $("#pessoa").val() == "fisica" ? $('input[name=cpf]').focus() : $('input[name=cnpj]').focus();
        returnCpfCnpj = 0;
      }
    });
    if (returnCpfCnpj == 0) return false;
    //FIM VALIDAÇÃO CPF/CNPJ
    
		var myList = [];
		
		if ($("#abas_fisica").parent().hasClass("ativo")){
			myList.push($('input[name=nome-completo]'));
			myList.push($('input[name=data-nascimento]'));
			myList.push($('input[name=cpf]'));
			myList.push($('input[name=rg]'));
			myList.push($('input[name=email-fisica]'));
			myList.push($('input[name=ddd-fisica]'));
			myList.push($('input[name=telefone-fisica]'));
		}else{
			myList.push($('input[name=razao-social]'));
			myList.push($('input[name=cnpj]'));
			myList.push($('input[name=ie]'));
			
			myList.push($('input[name=email-juridica]'));
			myList.push($('input[name=ddd-juridica]'));
			myList.push($('input[name=telefone-juridica]'));
		}
		
		myList.push($('input[name=cep1]'));
		myList.push($('input[name=cep2]'));
		myList.push($('input[name=endereco]'));
		myList.push($('input[name=endereco-numero]'));
		myList.push($('input[name=bairro]'));
		myList.push($('input[name=cidade]'));
		
		<?php
			if (strtoupper(@$_SESSION["PEDIDO"]["USUARIO"])=="NOVO"){
				echo "myList.push($('input[name=senha]'));";
				echo "myList.push($('input[name=confirmacao]'));";
			}
		?>
		
		for (var i=0; i < myList.length; i++) {
			var dad = myList[i].parent();
		...
		?>
				if ($('input[name=senha]').val()!=$('input[name=confirmacao]').val()){
					...
					$('input[name=senha]').parent().addClass("erro");
					$('input[name=confirmacao]').parent().addClass("erro");
		...
		//libera os campos para eles irem como POST
		$("#email-fisica").prop('disabled',false);
		$("#email-juridica").prop('disabled',false);
