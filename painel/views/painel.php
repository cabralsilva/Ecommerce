...
if (! isset ( $_SESSION ["PEDIDO"] ["USUARIO"] )) { // verifica se está logado
	...
	header ( "Location: " . UrlSite . BasePainel . "/loginPainel" );
	die ();
} elseif ($_SESSION ["PEDIDO"] ["USUARIO"] == "CADASTRADO" && isset ( $_SESSION ["PEDIDO"] ["codigo"] )) {
	// die();
	...
	<div class="carrinho painel">
		...
		<div class="conteudo">
			...
			<div class="limites">
				...
				<table class="compras ultimos-pedidos tablesaw tablesaw-stack"
...
if (count ( $_SESSION ["CLIENTE"] ["listaPedidos"] ) > 0) {
						foreach ( $_SESSION ["CLIENTE"] ["listaPedidos"] as $pedido ) {
							?>
						<!-- abre pedido -->
					...
					<tbody>
						<tr>
							...
							<td class="status"><svg width="24px" height="20px">
									<use xlink:href="#icone-carrinho" class="icone" /></svg> <br><?= $pedido["Processo"]?></td>
							<td>
								<!-- 									-- --> <!-- 									<a href="#" title="Ver nota fiscal eletrônica"><svg width="20px" height="23px"><use xlink:href="#icone-invoice" class="icone" /></svg></a> -->
							</td>
							<td><a href="#" class="botao" title="Ver detalhes do pedido">Detalhes
						...
						<tr data-tablesaw-no-labels>
							<td colspan="6">
								...
								<ul>
									<li><strong>Endereço:</strong> <?= $pedido["EntregaRua"] ?>, <?= $pedido["EntregaNumero"] ?> <?= ($pedido["EntregaComplemento"] != "") ? " - " . $pedido["EntregaComplemento"]:""?>
												<?= " - " . $pedido["EntregaBairro"] . " - " . $pedido["EntregaMunicipio"] . " - " . $pedido["EntregaUf"]?></li>
									<li><strong>CEP:</strong> <?= $pedido["EntregaCep"]?></li>
									<li><strong>Forma de Entrega:</strong> <?= $pedido["TipoFrete"]?></li>
									<li><strong>Previsão de Entrega:</strong> Indisponível</li>
									<li><strong>Rastrear Encomenda:</strong><a
										href="http://websro.correios.com.br/sro_bin/txect01$.QueryList?P_LINGUA=001&P_TIPO=001&P_COD_UNI=<?= $pedido["CodigoRastreio"]?>"
										target="_blank"> <?= $pedido["CodigoRastreio"] ?></a></li>
								</ul>
								...
								<div class="resumo-pedido">
									<table class="compras tablesaw tablesaw-stack"
										...
										<tbody>
												<?php foreach ($pedido["lstItens"] as $item){?>
													<tr>
												<th scope="row">
													<p>
														<a href="produto.html"
															title="Voltar à página deste produto"><?= $item["descricao"]?></a>
													</p>
												</th>
												<td>R$ <?= number_format($item["valorUnitario"],2,",","."); ?></td>
												<td><?= $item["quantidade"]?></td>
												<td>R$ <?= number_format($item["valorUnitario"] * $item["quantidade"],2,",","."); ?></td>
											</tr>
												<?php }?>
									...
									<table data-tablesaw-no-labels>
										<tfoot>
											<tr>
												<th scope="row">Frete</th>
												<td>R$ <?= number_format($pedido["ValorFrete"],2,",","."); ?></td>
											</tr>
											<tr>
												<th scope="row">Desconto</th>
												<td>R$ <?= number_format($pedido["ValorDesconto"],2,",","."); ?></td>
											</tr>
											<tr>
												<th scope="row">Total</th>
												<td>R$ <?= number_format($pedido["ValorFinal"] + $pedido["ValorFrete"],2,",","."); ?></td>
											</tr>
										</tfoot>
									</table>
								</div>
				...
				<a href="<?= UrlSite ?>" class="voltar" title="Voltar ao site"><svg
  ...
  185.615,206.828 350.018,371.23 371.23,350.018 206.828,185.615 " />                </symbol>
	...
	<script src="<?= UrlSite . BasePainel ?>/resources/js/main.min.js"
		defer></script>
	<!-- fecha scripts -->
