...
session_start();
// die();
if (isset($_SESSION ["PEDIDO"]["USUARIO"]) && $_SESSION ["PEDIDO"]["USUARIO"] == "CADASTRADO"){ //verifica se está logado
	echo "<script> parent.window.location.href='". UrlSite . BasePainel . "/pedidos'; </script>";
	die();
}elseif(isset($_SESSION["PEDIDO"]["USUARIO"]) && $_SESSION ["PEDIDO"]["USUARIO"] != "NOVO"){
	require_once '../util/IntegracaoSessao.php';
	$login = new Integracao();
	$login->logarPainelBySessaoCarrinho();
	die();
}
?>
<!DOCTYPE html>
...
<head>
	<meta charset="utf-8" />
	<title>Identificação - Carrinho de Compras - Plander.com</title>
	...
	<div class="carrinho painel">
		...
		<div class="conteudo">
			...
			<div class="limites">
				...
				<form action="<?= UrlSite . BasePainel ?>/controllers/loginPainelController.php" class="novo-cliente" method="post">
					<fieldset>
						<h3>Sou novo cliente, esta é minha primeira compra</h3>
						<label for="email-cadastro">Insira seu endereço de e-mail:</label>
						<input type="email" name="email-cadastro" id="email-cadastro" required="required">
						<label><input type="checkbox" name="newsletter"
							id="newsletter"> Sim, gostaria de receber novidades da
							Plander em meu e-mail.</input></label>
						<button type="submit">
							Continuar
				...
				<form action="<?= UrlSite . BasePainel ?>/controllers/loginPainelController.php" class="login" method="post">
					<fieldset>
						<h3>Já tenho cadastro</h3>
						<ol>
							<li><label for="email-login">Insira seu endereço de
									e-mail:</label><input type="email" name="email-login" id="email-login" required="required"></li>
							<li><label for="senha-login">Senha:</label><input
								class="pwd" type="password" name="senha-login" id="senha-login" required="required">
								<label><input type="checkbox" name="mostrar-senha"
									id="mostrar-senha"
									aria-label="Mostrar senha como texto aberto. Aviso: esta funcionalidade exibirá a senha abertamente em sua tela">
									Mostrar senha</label></li>
						</ol>
...
<!-- 						<a href="carrinho-sem-cadastro.html" class="botao continuar" -->
...
<!-- 				</div> -->
	...
	<svg style="display: none;">
		...
		<title>lupa</title>  <g id="busca-Desktop" fill-rule="evenodd"
			sketch:type="MSPage" fill="none"> <g
			id="busca-P&#xE1;gina-Inicial" transform="translate(-926 -139)"
			fill="currentColor" sketch:type="MSArtboardGroup"> <g
			id="busca-topo" sketch:type="MSLayerGroup"> <g id="busca-busca"
			sketch:type="MSShapeGroup" transform="translate(388 128)"> <g
			id="busca-lupa" transform="translate(538 11)"> <g
			id="busca-Group" transform="translate(.000626)"> <path
			id="busca-Shape"
  ...
  185.615,206.828 350.018,371.23 371.23,350.018 206.828,185.615 " />                </symbol>
		...
		<title>truck</title>  <g id="frete-Desktop" fill-rule="evenodd"
			sketch:type="MSPage" fill="none"> <g
			id="frete-P&#xE1;gina-Inicial" transform="translate(-156 -1209)"
			fill="currentColor" sketch:type="MSArtboardGroup"> <g
			id="frete-galeria-produtos" transform="translate(34 816)"
			sketch:type="MSLayerGroup"> <g id="frete-produto"
			sketch:type="MSShapeGroup"> <g id="frete-selo-frete"
			transform="translate(122 392)"> <g id="frete-truck"
			transform="translate(0 1)"> <path id="frete-Shape"
		...
		<title>I</title>  <g stroke="none" stroke-width="1" fill="none"
			fill-rule="evenodd"> <g
			id="invoice-Painel-do-usu&#xE1;rio---Pedidos"
			transform="translate(-985.000000, -350.000000)" fill="currentColor"> <g
			id="invoice-conte&#xFA;do"
			transform="translate(27.000000, 111.000000)"> <g
			id="invoice-pedido" transform="translate(0.000000, 167.000000)"> <g
			id="invoice-tabela"> <g id="invoice-item-aberto"
			transform="translate(0.000000, 43.000000)"> <g
		...
		<title>Voltar</title>  <g stroke="none" stroke-width="1" fill="none"
			...
			transform="translate(27.000000, 111.000000)"> <g
			id="seta-voltar-nav" transform="translate(662.000000, 17.000000)"> <g
		...
		<title>Shape</title>  <g id="seta-Desktop" fill-rule="evenodd"
			sketch:type="MSPage" fill="none"> <g
			id="seta-P&#xE1;gina-Inicial" transform="translate(-32 -712)"
			fill="currentColor" sketch:type="MSArtboardGroup"> <g
			id="seta-marcas" transform="translate(32 698)"
			sketch:type="MSLayerGroup"> <g id="seta-arrow395"
			sketch:type="MSShapeGroup" transform="translate(0 14)"> <path
			id="seta-Shape"
		...
		<title>Shape</title>  <g id="telefone-Desktop" fill-rule="evenodd"
			...
			fill="currentColor" sketch:type="MSArtboardGroup"> <g
			id="telefone-topo" sketch:type="MSLayerGroup"> <g
			id="telefone-nav" sketch:type="MSShapeGroup"
			transform="translate(28 25)"> <g id="telefone-telefone"
			transform="translate(757.5 3.1855)"> <path id="telefone-Shape"
	...
	<!-- abre scripts -->
	<script src="<?= UrlSite . BasePainel ?>/resources/bower_components/jquery/dist/jquery.min.js"></script>
	...
	<script>
		$(document).ready(function(){
			$("#esqueci").click(function(){
				if ($("#email-login").val()!="")
					window.location.href="<?= UrlSite . BasePainel ?>/controllers/loginPainelController.php?acao=esqueci&email="+$("#email-login").val();
				else{
