(function(){
  ...
  // Painel do Usuário
  if (document.querySelector('table.ultimos-pedidos') != null) {
    var botaoDetalhes = document.querySelectorAll('table.ultimos-pedidos a.botao');
    var className = 'ativo';

    Array.prototype.forEach.call(botaoDetalhes, function(el, i){
      var resultado = el.parentNode.parentNode;

      el.addEventListener('click', function(e){
        if (resultado.classList) {
          resultado.classList.toggle(className);
        } else {
          var classes = resultado.className.split(' ');
          var existingIndex = classes.indexOf(className);
          ...
          resultado.className = classes.join(' ');
        }
