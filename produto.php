<?php
	...
	$_redirecionar = false; //pra quando vem de parametro apenas o codigo, ele direciona pra produto/descricao/codigo
	
	if (@$_GET["cod"]!="" && @$_GET["cods"]!=""){
		$_codProduto = @$_GET["cods"];
		$_codProdutoGrade = @$_GET["cod"];
	}else{
		$_codProduto = @$_GET["cod_prod"];
		$_codProdutoGrade = @$_GET["cod_prod_grade"];
		
		$_redirecionar = @$_GET["descricao"]=="";
	}
	...
	$_sqlProdutoGrade = "SELECT PG.Codigo, PG.Id AS Id_ProdutoGrade, 
								...
								P.Promocao, 
								P.ValorSaida, 
								P.ValorSaidaPromocao, 
								P.QuantidadeParcelas, 
								P.QuantidadeParcelasSemJuros, 
								P.Transportadora, 
								P.PesoBruto, 
								P.Altura, 
								P.Largura, 
								...
								P.TituloPagina,
								P.DescricaoPagina,
								P.PalavraChavePagina 
								FROM Produto AS P 
								INNER JOIN ProdutoGrade AS PG ON P.Codigo = PG.CodigoProduto 
								INNER JOIN ProdutoCategoria AS PC ON P.Codigo = PC.CodigoProduto
								INNER JOIN Categoria AS C ON C.Codigo = PC.CodigoCategoria and C.Liberado='Liberado'
								INNER JOIN Grupo AS G ON G.Codigo = C.CodigoGrupo and G.Liberado='Liberado'
								WHERE P.Liberado = 'Liberado' and PG.CodigoProdutoGrade = '" . $_codProdutoGrade . "' ORDER BY PG.DescricaoGradeWeb ASC";
	$_queryProdutoGrade = mysql_query($_sqlProdutoGrade);
	
	$_encontrado = mysql_num_rows($_queryProdutoGrade);
	
	$_produto = array();
	
	if ($_encontrado==0)
		die("<script> alert('Produto nao encontrado!'); window.location.href='" . URL . "index'; </script>");
	
	while($_resultProdutoGrade = mysql_fetch_array($_queryProdutoGrade)) {
		$_produto = array(
							"Codigo"=>$_resultProdutoGrade['Codigo'],
							"codigoProdutoGrade"=>$_resultProdutoGrade['CodigoProdutoGrade'],
							"Id"=>$_resultProdutoGrade['Id_ProdutoGrade'],
							"Descricao"=>$_resultProdutoGrade['Descricao'],
							"CodigoProduto"=>$_resultProdutoGrade['CodigoProduto'],
							"Especificacao"=>$_resultProdutoGrade['Especificacao'],
							"Fabricante"=>$_resultProdutoGrade['CodigoFabricante'],
							"CodigoCategoria"=>$_resultProdutoGrade['CodigoCategoria'],
							"Categoria"=>$_resultProdutoGrade['Categoria'],
							"CodigoGrupo"=>$_resultProdutoGrade['CodigoGrupo'],
							"Grupo"=>$_resultProdutoGrade['Grupo'],
							"Tipo"=>$_resultProdutoGrade['Tipo'],
							"Departamento"=>$_resultProdutoGrade['Departamento'],
							"Disponivel"=>$_resultProdutoGrade['Disponivel'],
							"Modelo"=>$_resultProdutoGrade['Modelo'],
							"Promocao"=>$_resultProdutoGrade['Promocao'],
							"ValorSaida"=>$_resultProdutoGrade['ValorSaida'],
							"ValorSaidaPromocao"=>$_resultProdutoGrade['ValorSaidaPromocao'],
							"QuantidadeParcelas"=>$_resultProdutoGrade['QuantidadeParcelas'],
							"QuantidadeParcelasSemJuros"=>$_resultProdutoGrade['QuantidadeParcelasSemJuros'],
							"Transportadora"=>$_resultProdutoGrade['Transportadora'],	
							"IdProduto"=>$_resultProdutoGrade['Id_Produto'],
							"PesoBruto"=>$_resultProdutoGrade['PesoBruto'],
							"Altura"=>$_resultProdutoGrade['Altura'],
							"Largura"=>$_resultProdutoGrade['Largura'],
							"Comprimento"=>$_resultProdutoGrade['Comprimento'],
							"TituloPagina"=>$_resultProdutoGrade['TituloPagina'],
							"DescricaoPagina"=>$_resultProdutoGrade['DescricaoPagina'],
							"PalavraChavePagina"=>$_resultProdutoGrade['PalavraChavePagina']
			);
	}
	
	if ($_redirecionar)
		die("<script> window.location.href='" . URL . "produto/" . geraDescricaoAmigavel($_produto["Descricao"]) . "/" . $_codProdutoGrade . "'; </script>");
	
	//***********************************************************************************
	
	$coda = "produtos/" . $_codProdutoGrade;
	@$_files = glob("{$coda}/*.jpg");
	count(@$_files);
	$total = strlen(@$_files[0]);
	if( $total == 0){
		unset ($_files);
	}
	...
	$_tituloPagina = $_produto["TituloPagina"];
	$_descricaoPagina = $_produto["DescricaoPagina"];
	$_palavraChavePagina = $_produto["PalavraChavePagina"];
	$_tituloDescricao = $_produto["Descricao"];
	
	//*********************************************************************************************
	
	$_breadcrumb = array();
	
	$_li = "<li itemprop=\"itemListElement\" itemscope itemtype=\"http://schema.org/ListItem\">";
	$_li .= "<a itemprop=\"item\" href=\"targetLink\">";
	$_li .= "<span itemprop=\"name\">targetNome</span></a>";
    $_li .= "<meta itemprop=\"position\" content=\"targetContent\" />";
    $_li .= "</li>";
	
	//DEPARTAMENTO
	$_texto = $_li;
	$_texto = str_replace("targetNome", mb_strtoupper($_produto["Departamento"],"UTF-8"), $_texto);
	$_texto = str_replace("targetLink", URL ."produtos/depar/" . removerAcento($_produto["Departamento"]), $_texto);
	$_breadcrumb[count($_breadcrumb)] = $_texto;
	
	//TIPO
	$_texto = $_li;
	$_texto = str_replace("targetNome", mb_strtoupper($_produto["Tipo"],"UTF-8"), $_texto);
	$_texto = str_replace("targetLink", URL ."produtos/tipo/" . removerAcento($_produto["Departamento"]) . "/" . removerAcento($_produto["Tipo"]), $_texto);
	$_breadcrumb[count($_breadcrumb)] = $_texto;
	
	//GRUPO
	$_link = URL ."produtos/grupo/" . removerAcento($_produto["Departamento"]) . "/" . removerAcento($_produto["Tipo"]);
	$_link .= "/" . removerAcento($_produto["Grupo"]) . "/" . $_produto["CodigoGrupo"];
	$_texto = $_li;
	$_texto = str_replace("targetNome", mb_strtoupper($_produto["Grupo"],"UTF-8"), $_texto);
	$_texto = str_replace("targetLink", $_link, $_texto);
	$_breadcrumb[count($_breadcrumb)] = $_texto;
	
	if ($_produto["Grupo"]!=$_produto["Categoria"]){
		//CATEGORIA
		$_link = URL ."produtos/categoria/" . removerAcento($_produto["Departamento"]) . "/" . removerAcento($_produto["Tipo"]);
		$_link .= "/" . removerAcento($_produto["Grupo"]) . "/" . $_produto["CodigoGrupo"];
		$_link .= "/" . removerAcento($_produto["Categoria"]) . "/" . $_produto["CodigoCategoria"];
		$_texto = $_li;
		$_texto = str_replace("targetNome", mb_strtoupper($_produto["Categoria"],"UTF-8"), $_texto);
		$_texto = str_replace("targetLink", $_link, $_texto);
		$_breadcrumb[count($_breadcrumb)] = $_texto;
	}
  ...
  <div class="limites">
    ...
    <ol class="breadcrumb" itemscope itemtype="http://schema.org/BreadcrumbList">
      <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
        <a itemprop="item" href="<?php echo URL; ?>index.php"><span itemprop="name">plander.com</span></a>
        <meta itemprop="position" content="1" />
      </li>
      <?php
      	for ($_i=0; $_i < count($_breadcrumb); $_i++) { 
			  echo str_replace("targContent", ($_i+1), $_breadcrumb[$_i]);
		  }
      ?>
      <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
        <span itemprop="name"><?php echo $_produto['Descricao']; ?></span>
        <meta itemprop="position" content="3" />
      </li>
    </ol>

    <!-- abre section.detalhes-produto -->
    <section class="detalhes-produto" itemscope itemtype="http://schema.org/Product">
      <h2 itemprop="name"><?php echo $_produto['Descricao']; ?></h2>
      <div class="imagem">
      	<?php
          	if (count(@$_files)>0){
        		echo "<a href=\"" . URL . "" . @$_files[0] . "\" title=\"" . $_produto['Descricao'] . "\">";
        		echo "<img id=\"ampliada\" src=\"" . URL . "actions/img.php?l=384&a=384&local=../" . @$_files[0] . "\" data-zoom-image=\"" . URL . "" .@$_files[0] . "\" alt=\"" . $_produto['Descricao'] . "\" itemprop=\"image\">";
        		echo "</a>";
        	}else{
        		echo "<a href=\"" . URL . "imagens/sem_imagem.jpg\" title=\"" . $_produto['Descricao'] . "\">";
        		echo "<img id=\"ampliada\" src=\"" . URL . "actions/img.php?l=384&a=384&local=../imagens/sem_imagem.jpg\" data-zoom-image=\"imagens/sem_imagem.jpg\" data-zoom-image=\"" . URL . "imagens/sem_imagem.jpg\" alt=\"" . $_produto['Descricao'] . "\" itemprop=\"image\">";
        		echo "</a>";
        	}
        ?>
        <p>Passe o cursor sobre a imagem para ampliar</p>
          	...
          	<?php
          		if (count(@$_files)>0){
	          		for ($_i=0; $_i < count(@$_files); $_i++) { 
						...
						echo "<a " . ($_i==0?"class=\"active\"":"") . " href=\"" . URL . "" . @$_files[$_i] . "\" ";
						echo "data-image=\"" . URL . "actions/img.php?l=384&a=384&local=../" . @$_files[$_i] . "\" ";
						echo "data-zoom-image=\"" . URL . "" . @$_files[$_i] . "\">";
						echo "<img src=\"" . URL . "actions/img.php?l=202&a=194&local=../" . @$_files[$_i] . "\" alt=\"" . $_produto["Descricao"] . "\">";
						echo "</a>";
						echo "</div>";
					}
				}else{
					echo "<div class=\"swiper-slide\">";
					echo "<a " . ($_i==0?"class=\"active\"":"") . " href=\"#\" ";
					echo "data-image=\"" . URL . "actions/img.php?l=384&a=384&local=../imagens/sem_imagem.jpg\" ";
					echo "data-zoom-image=\"" . URL . "imagens/sem_imagem.jpg\">";
					echo "<img src=\"" . URL . "actions/img.php?l=202&a=194&local=../imagens/sem_imagem.jpg\" alt=\"" . $_produto["Descricao"] . "\">";
					echo "</a>";
					echo "</div>";
				}
          	?>
      ...
      <div class="dados">
        <div itemprop="offers" itemscope itemtype="http://schema.org/Offer">
          <?php
          	if ($_produto["Promocao"]=="Promoção"){
          		echo "<span class=\"de\"><span>de R$ " . number_format($_produto["ValorSaida"],2,",",".") . "</span></span>";
          		echo "<span class=\"por\">por <span itemprop=\"price\">R$ " . number_format($_produto["ValorSaidaPromocao"],2,",",".") . "</span></span>";
			}else
				echo "<span class=\"por\"><span itemprop=\"price\">R$ " . number_format($_produto["ValorSaida"],2,",",".") . "</span></span>";
			
			$_quantParcelas = 1;
			$_quantParcelasSJ = 1;
			
			if($_produto["QuantidadeParcelas"]>0)
                $_quantParcelas = $_produto["QuantidadeParcelas"];
            
            if($_produto["QuantidadeParcelasSemJuros"]>0)
                $_quantParcelasSJ = $_produto["QuantidadeParcelasSemJuros"];
            
			echo "<em>em até " . $_quantParcelasSJ . "x de " . number_format($_produto["ValorSaidaPromocao"]/$_quantParcelasSJ, 2, ",", ".") . " sem juros</em>";
          ?>
          ...
          <ul class="parcelas">
          	<?php
                ...
                for($_i=0; $_i<$_quantParcelas; $_i++) {
                	$_valor = $_produto["Promocao"]=="Promoção"?$_produto["ValorSaidaPromocao"]:$_produto["ValorSaida"];
					
                	echo "<li>";
                	echo "<span>";
                	echo ($_i+1) . "x " . $_juros[$_i<$_quantParcelasSJ] . " de";
                	echo "</span>";
					echo " <span>";
					
					if($_i<$_quantParcelasSJ)
                		echo "R$ " . number_format($_valor / ($_i+1), 2, ",", ".");
					else
						echo "R$ " . number_format(pmt(JUROS,($_i+1),$_valor), 2, ",", ".");
        ...
        <div class="compra">
        	<!--<input type="image" onClick="$('.op').load('include/listagemCarrinho.php?acao=inserir&id=<?php echo $rs_produto_grade['Id_Produto'];?>&idprod=<?= $_SESSION["id"];?>&cod=<? echo $_SESSION["cod"]; ?>');" src="images/_plander_produtos_viola_v3_03.png" width="101" height="29" border="0" style="margin-top:-30px; margin-left:225px;">-->
          <?php
          	if ($_produto["Disponivel"]>0)
          		echo "<a href=\"" . URL_SSL . "includes/listagemCarrinho.php?acao=inserir&id=" . $_produto["Id"] . "\" class=\"botao action\" title=\"Comprar este produto\">Comprar <svg width=\"8px\" height=\"15px\"><use xlink:href=\"#icone-seta\" class=\"icone\" /></svg></a>";
			else
          		echo "<h3 class=\"indisponivel\"><svg width=\"48px\" height=\"45px\"><use xlink:href=\"#icone-alerta\" class=\"icone\" /></svg> Produto indisponível</h3>";
		  ?>
          <p class="seguranca centro"><img src="<?php echo URL; ?>imagens/compra-segura.jpg" alt="Compra segura"><img src="<?php echo URL; ?>imagens/satisfacao.jpg" alt="Satisfação Garantida"></p>
          <a href="#" class="indique" title="Indique este produto para um amigo"><svg width="24px" height="17px"><use xlink:href="#icone-mail" class="icone" /></svg> Indique para um amigo</a>
          <div class="dropdown">
          	<iframe name="iframeOp" style="display:none"></iframe>
            <form target="iframeOp" action="<?php echo URL; ?>actions/indiqueProduto.php?cod_prod_grade=<?php echo $_codProdutoGrade; ?>" method="POST" onsubmit="return mySubmit();">
              <fieldset>
                <p>Digite os dados do seu amigo para que a indicação seja enviada por e-mail:</p>
                <ol>
                  <li><label for="nome-amigo">Nome do amigo:</label><input type="text" name="nome-amigo" id="nome-amigo" required="required"></li>
                  <li><label for="email-amigo">E-mail do amigo:</label><input type="text" name="email-amigo" id="email-amigo" required="required"></li>
                  <li><label for="mensagem-indicacao">Mensagem:</label><textarea name="mensagem-indicacao" id="mensagem-indicacao" rows="5" cols="30" required="required"></textarea></li>
                  <li><label for="seu-nome">Seu nome:</label><input type="text" name="seu-nome" id="seu-nome" required="required"></li>
                  <li><label for="seu-email">Seu e-mail:</label><input type="text" name="seu-email" id="seu-email" required="required"></li>
                  <li><label for="seu-email">Preencha com os caracteres: <img id="img-captcha" /></label><input type="text" name="code" id="code" required="required"></li>
                </ol>
        ...
        <div class="cep">
          <form action="#" onsubmit="return false">
            <fieldset>
              ...
              <p>
              	<input class="grande" type="text" name="cep-inicial" id="cep-inicial" maxlength="5">
              	<input class="pequeno" type="text" name="cep-final" id="cep-final" maxlength="3">
          ...
          <div class="resultado">
            <!--<p>Av. Sete de Setembro, Batel - Curitiba/PR</p>-->
            <table>
              <tbody>
              </tbody>
            </table>
      ...
      <div class="descricao">
        <h2>Descrição</h2>
        <?php
        	echo nl2br($_produto["Especificacao"]);
        ?>
        <!--
        <div class="video-embed">
          <iframe width="560" height="315" src="https://www.youtube.com/embed/B2JLvi3rovs" frameborder="0" allowfullscreen></iframe>
        </div>
        <p class="centro"><strong><small>Imagens meramente ilustrativas.</small></strong> <small>Todas as informações divulgadas são de responsabilidade do fabricante/fornecedor.</small></p>
        ...
        <table class="informacoes">
          <tbody>
            ...
            <tr>
              <th scope="row">Braço</th>
              <td>Nato</td>
            </tr>
            <tr>
              <th scope="row">Cor</th>
              <td>Natural, natural com acabamento em Gloss</td>
            </tr>
            <tr>
              <th scope="row">Cordas</th>
              <td>Aço</td>
            </tr>
            <tr>
            ...
            <tr>
              <th scope="row">Faixa e fundo</th>
              <td>Meranti</td>
            </tr>
            <tr>
              <th scope="row">Marcação</th>
              <td>Bolinhas brancas</td>
            </tr>
            <tr>
              <th scope="row">Tampo</th>
              <td>Spruce</td>
            </tr>
            <tr>
              <th scope="row">Tarraxas</th>
              <td>Cromadas</td>
            </tr>
            <tr>
            ...
            <tr>
              <th scope="row">Tipo de Garantia</th>
              <td>Análise</td>
            </tr>
            <tr>
              <th scope="row">Tempo de Garantia</th>
              <td>12 meses</td>
            </tr>
          </tbody>
        </table>
    ...
    <!-- fecha section.detalhes-produto -->
...
<script>
	$(document).ready(function(){
		document.getElementById("img-captcha").setAttribute('src', '<?php echo URL; ?>captcha/captcha.php');
	    ...
	    $(".cep form fieldset p button").click(function() {
	    	if ($("#cep-inicial").val() == "" || $("#cep-final").val() == "")
				...
				var transportadora = "<?php echo $_produto["Transportadora"]; ?>";
				if(transportadora == "Transportadora"){
					var strTransp = "<tr>";
					strTransp += "<td>ENTREGA EXCLUSIVA POR TRANSPORTADORA!</td>";
	                strTransp += "</tr>";
	                strTransp += "<tr>";
					strTransp += "<td>CONSULTE A LOJA PARA VER TAXA E PREÇOS DE ENTREGA PELO TELEFONE <?php echo TELEFONE; ?> OU PELO E-MAIL plander@plander.com.br</td>";
	                strTransp += "</tr>";
	                
	                $(".resultado").slideDown();
	                $(".resultado table tbody").append(strTransp);
				}else{
					var cep = $("#cep-inicial").val() + $("#cep-final").val();
					
					var dados = {
									"tipo":"0",
									'cep':cep,
									'peso':'<?php echo $_produto["PesoBruto"]; ?>',
									'altura':'<?php echo $_produto["Altura"]; ?>',
									'largura':'<?php echo $_produto["Largura"]; ?>',
									'comprimento':'<?php echo $_produto["Comprimento"]; ?>'
									}
					
					$.ajax({
						url: "<?php echo URL; ?>actions/calcularFrete.php",
						data: dados,
						...
						success: function(frete){
								if(frete[0] == "n"){
									alert("Ocorreu algum erro durante o cálculo do frete! Tente novamente!");
								}else{
									
									var strPac = "<tr>";
									strPac += "<th scope=\"row\">R$ " + frete.valor_pac + "</th>";
									strPac += "<td>PAC</td>";
									strPac += "<td>" + frete.prazo_pac + " dias úteis após a postagem</td>";
									strPac += "</tr>";
									
									var strSedex = "<tr>";
									strSedex += "<th scope=\"row\">R$ " + frete.valor_sedex + "</th>";
									strSedex += "<td>SEDEX</td>";
									strSedex += "<td>" + frete.prazo_sedex + " dias úteis após a postagem</td>";
									strSedex += "</tr>";
									
									$(".resultado").slideDown();
									$(".resultado table tbody").append(strPac);
									$(".resultado table tbody").append(strSedex);
	...
	function mySubmit(){
		...
		var sessao = "<?php print $_SESSION["security_code"]; ?>";
		var input = document.getElementById("code");
		if (sessao != input.value){
			alert("Codigo de Verificação INVÁLIDO!");
			input.focus();
			return false;
		}
	}
