...
ServerConfigurationClass.prototype.showServerConfiguration = function() {
    ...
    $('#sc-apply-btn').click(function() {
        if(!lzm_chatServerEvaluation.m_ServerConfigBlocked)
            that.uploadToServer();
    });
    ...
    this.m_Config.m_Settings.forEach(function(entry_name) {
        tabs.push({name: entry_name.title, content: lzm_displayLayout.getForm(entry_name.name, that.m_Config, 'sc'), hash: entry_name.name});
    });
...
ServerConfigurationClass.prototype.setValidation = function(id) {
    var input = lzm_chatServerEvaluation.inputList.getCustomInput(id,'id');
...
ServerConfigurationClass.prototype.uploadToServer = function(_serverTestId) {
    ...
    lzm_chatServerEvaluation.m_ServerConfigBlocked = true;
    $('#main-menu-panel-tools-configuration').addClass('ui-disabled');
    lzm_chatPollServer.pollServerDiscrete('set_config',data,true).done(function(data) {
...
ServerConfigurationClass.prototype.getPostData = function() {
    ...
    // inputs
    for (var key in lzm_chatServerEvaluation.inputList.objects)
    {
        if(key.length)
        {
            var obj = lzm_chatServerEvaluation.inputList.objects[key];
            lzm_chatServerEvaluation.inputList.applyInputHtmlRow(obj);
            data['p_cfg_g_gl_input_list_' + key] = lz_global_base64_encode(lzm_chatServerEvaluation.inputList.phpSerializeInput(obj));
        }
    }
    ...
    $("#email-acc-outgoing tbody tr").each(function() {
        if($(this).attr("data-obj")){
            ...
            data['p_cfg_es_a_' + count] = ea.auth ? 'Yes' : 'No';
            data['p_cfg_es_d_' + count] = '-1';
            data['p_cfg_es_t_' + count] = ea.type;
            data['p_cfg_es_sn_' + count] = ea.senderName;
            data['p_cfg_es_de_' + count] = ea.default ? '1' : ($("#email-acc-outgoing tbody").length<=1 ? '1' : '0');
    ...
    this.m_Config.m_Settings.forEach(function(entry_name) {
        entry_name.groups.forEach(function(entry_group) {
            entry_group.controls.forEach(function(entry_control) {
                ...
                {
                    var value = lzm_displayLayout.GetControlValue(entry_control);
    ...
    data['p_cfg_g_gl_keywords'] = lz_global_base64_encode('LiveZilla, Live, Support, Customer Support, Live Chat, Live Help');
    ...
    // pass thru
    var ptignore =['gl_db_host','gl_db_user','gl_db_ext','gl_db_eng','gl_db_pass','gl_db_name','gl_db_prefix','wcl_upload_max_filesize','wcl_geo_tracking','ss_mnos','gl_teh','gl_stmo','gl_prem','gl_log','gl_input_list','gl_cbcd','gl_atrs','gl_root','gl_datprov','gl_insu','gl_insp'];
    var ptallow = ['gl_usrsp','gl_st_agdo','gl_st_atrc','gl_st_darp','gl_st_davl','gl_st_dbhc','gl_st_dbhv','gl_st_dboa','gl_st_dboc','gl_st_dbof','gl_st_dboi','gl_st_dbot','gl_st_derd','gl_st_dere','gl_st_ders','gl_st_drch','gl_st_dtdo','gl_st_dtee','gl_st_dtis','gl_st_dtkb','gl_st_dtor','gl_st_dtpa','gl_st_dtre','gl_st_dtse','gl_st_dtsy','gl_st_dtvi','gl_st_getp','gl_st_hifi','gl_st_marp','gl_st_mbdc','gl_st_mbdv','gl_st_mbhc','gl_st_mbhv','gl_st_mboa','gl_st_mboc','gl_st_mbof','gl_st_mboi','gl_st_mbot','gl_st_mtdo','gl_st_mtee','gl_st_mtis','gl_st_mtkb','gl_st_mtor','gl_st_mtpa','gl_st_mtre','gl_st_mtse','gl_st_mtsy','gl_st_mtvi','gl_st_muvl','gl_st_ropr','gl_st_toam','gl_st_upin','gl_st_yarp','gl_st_ybhc','gl_st_ybhv','gl_st_ybmc','gl_st_ybmv','gl_st_yboa','gl_st_yboc','gl_st_ybof','gl_st_yboi','gl_st_ybot','gl_st_ytdo','gl_st_ytee','gl_st_ytis','gl_st_ytkb','gl_st_ytor','gl_st_ytpa','gl_st_ytre','gl_st_ytse','gl_st_ytsy','gl_st_ytvi','gl_sctl','gl_lcut','gl_gtl','gl_gt_us','gl_gt_v','gl_gt_vc','gl_gt_vs','gl_gt_vsc','gl_gtdb','gl_ftpf','gl_ccac','gl_ccsv','gl_ccva'];

    for (var i=0; i<lzm_chatServerEvaluation.global_configuration.site[0].length; i++) {
        if (typeof lzm_chatServerEvaluation.global_configuration.site[0][i].subkeys != 'undefined') {
            var key = lzm_chatServerEvaluation.global_configuration.site[0][i].key;
            if(typeof data['p_cfg_g_' + key] == 'undefined' && $.inArray(key,ptallow)!=-1){
                data['p_cfg_g_' + key] = lz_global_base64_encode(lzm_chatServerEvaluation.global_configuration.site[0][i].value);
            }
            //else if(typeof data['p_cfg_g_' + key] == 'undefined' && $.inArray(key,ptignore)===-1)
              //
        }
    }
...
ServerConfigurationClass.prototype.createBackLink = function(type){
    var bl = "";
    lzm_chatServerEvaluation.m_BacklinkIndex = (this.licenseExists('NBL','type')) ? 2 : 0;
    if (type == 'Email'){
        if (lzm_chatServerEvaluation.m_BacklinkIndex != 2)
            bl = "Powered by LiveZilla Help Desk Software [http://www.livezilla.net]";
        else if (lzm_chatServerEvaluation.m_BacklinkTitle.length > 0)
            bl += lzm_chatServerEvaluation.m_BacklinkTitle + " [" + lzm_chatServerEvaluation.m_BacklinkUrl + "]";
        else
            return '';
    }
    else if (type == 'Short'){
        ...
        if (lzm_chatServerEvaluation.m_BacklinkIndex == 2){
            if (lzm_chatServerEvaluation.m_BacklinkTitle.length > 0 && lzm_chatServerEvaluation.m_BacklinkTitle.length < 16)
                bl += lzm_chatServerEvaluation.m_BacklinkUrl + "\" target=\"_blank\">" + lzm_chatServerEvaluation.m_BacklinkTitle.replace("Powered by ", "").replace(" Software", "");
            else if (lzm_chatServerEvaluation.m_BacklinkTitle.length > 0)
                bl += lzm_chatServerEvaluation.m_BacklinkUrl + "\" target=\"_blank\">" + "LiveZilla";
            else
                return '';
        }
        else{
            bl += "http://www.livezilla.net/\" title=\"Powered by LiveZilla\" target=\"_blank\">";
        ...
        if (lzm_chatServerEvaluation.m_BacklinkIndex == 2){
            if (lzm_chatServerEvaluation.m_BacklinkTitle.length > 0)
                bl += lzm_chatServerEvaluation.m_BacklinkUrl + "\" target=\"_blank\">" + lzm_chatServerEvaluation.m_BacklinkTitle;
            else
                return '';
        }
        else{
            bl += "http://www.livezilla.net/\" target=\"_blank\">";
            bl += ServerConfigurationClass.m_Backlinks[lzm_chatServerEvaluation.m_BacklinkIndex];
            bl += this.getBackLinkTitle(type);
        }
        bl += "</a>";
    }
    else if (type == 'Brand' && lzm_chatServerEvaluation.m_BacklinkIndex == 0){
...
ServerConfigurationClass.prototype.getBackLinkTitle = function(type){
    var titles;
    if (type == 'Short')
        titles = [" Live Chat", " Live Help", " Live Help"];
    else if (type == 'Brand')
        titles = [""];
    else
...
ServerConfigurationClass.prototype.removeLicenseKey = function (hash) {
    ...
    $("#sc-license-list tr").each(function() {
        if($(this).attr("data-hash")==hash)
        {
            var that = $(this);
            var myDataObject = {};
            myDataObject['p_server'] = lzm_chatServerEvaluation.getConfigValue('gl_lzid',true).value;
            ...
            $.ajax({
                type: "POST",
                url: "https://www.livezilla.net/premium/client/",
                data: myDataObject,
                timeout: 15000,
                dataType: 'text'
            }).done(function(data) {
...
ServerConfigurationClass.prototype.configureLicense = function (type) {
    ...
    if(type=='NBL')
    {
        confHtml += '<div style="width:350px;" class="top-space">' + lzm_inputControls.createCheckbox('sc-cfl-nbl-link-visible',tid('link_visible'),lzm_chatServerEvaluation.m_BacklinkTitle.length>0,'') + '</div>';
        confHtml += '<div class="top-space left-space-child">' + lzm_inputControls.createInput('sc-cfl-nbl-link-title','sc-cfl-nbl-link',lzm_chatServerEvaluation.m_BacklinkTitle,tid('link_title'),'','text','') + '</div>';
        confHtml += '<div class="top-space left-space-child">' + lzm_inputControls.createInput('sc-cfl-nbl-link-url','sc-cfl-nbl-link',lzm_chatServerEvaluation.m_BacklinkUrl,tid('url'),'','text','') + '</div>';
        ...
        $('#sc-cfl-nbl-link-title').attr("maxlength",15);
        $('#sc-cfl-nbl-link-title').val(lzm_chatServerEvaluation.m_BacklinkTitle);
        $('#sc-cfl-nbl-link-visible').change();

        $('#alert-btn-clok').click(function() {
            if($('#sc-cfl-nbl-link-visible').prop('checked')){
                lzm_chatServerEvaluation.m_BacklinkUrl = $('#sc-cfl-nbl-link-url').val();
                lzm_chatServerEvaluation.m_BacklinkTitle = $('#sc-cfl-nbl-link-title').val();
            }
            else {
                lzm_chatServerEvaluation.m_BacklinkUrl =
                lzm_chatServerEvaluation.m_BacklinkTitle = '';
            }
            lzm_commonDialog.removeAlertDialog();
        });
    ...
    $('#alert-btn-ok').click(function() {
        ...
        if(lk.length == 23){
            ...
            $.ajax({
                ...
                dataType: 'text'
                    ...
                    $(xmlDoc).find('pob').each(function(){
                        ...
                        if (lmajreq > -1 && lmajavl > -1 && lmajreq > lmajavl)
                        {
                            ...
                            // TRIAL
                            //lhash = 'fbef327921d94a0100af0cc67e45649e';
                            //lserial = 'TRIAL';
                            //YToyOntpOjA7czo0NDoiWm1KbFpqTXlOemt5TVdRNU5HRXdNVEF3WVdZd1kyTTJOMlUwTlRZME9XVT0iO2k6MTtzOjg6IlZGSkpRVXc9Ijt9
                            ...
                            if(ltypek == 'OPR' && !that.licenseExists('NGL','type')){
                                rowHtml = that.m_Config.GetLicenseRow('<b class="text-orange">'+tid('new')+'</b>',1,'Geo Tracking','NGL','-',lzm_chatServerEvaluation.getHash('NGL',lzm_chatServerEvaluation.getConfigValue('gl_lzid',true).value), false, false);
                                $('#sc-license-list tbody tr:first').before(rowHtml);
                            }
                ...
                }).fail(function(jqXHR, textStatus, errorThrown){alert(textStatus);lzm_commonDialog.removeAlertDialog();});
        }
        else
        {
            lzm_commonDialog.removeAlertDialog();
...
ServerConfigurationClass.prototype.emailAction = function(action, type, id) {
    if(action == 'remove')
        ...
        var ea = {id:id,senderName:'',email:'',default:false,boxtype:type,host:'',userName:'',type:'SMTP',password:'',encrypt:'',port:25, auth:true,framework:'ZEND',frequency:10};
        if(action == 'edit')
        ...
        lzm_commonDialog.createAlertDialog(lzm_chatDisplay.ServerConfigurationClass.getEmailAccountForm(ea), [{id: 'eaok', name: tid('ok')}, {id: 'eacancel', name: tid('cancel')},{id: 'eatest', name: tid('test')},{id: 'eahelp', name: tid('help')}],true,true,true);
        ...
        $('#alert-btn-eahelp').click(function() {
...
ServerConfigurationClass.prototype.testLDAP = function(){
    ...
    lzm_chatPollServer.pollServerDiscrete('ldap_test',pdata).done(function(data) {
        try{
...
ServerConfigurationClass.prototype.runEmailTest = function(id) {
    ...
    var message = "";
    lzm_chatPollServer.pollServerDiscrete('send_test_mail',{'p_mailbox':id}).done(function(data) {
...
function ServerConfiguration(_cloneObj) {
    ...
    var defLang = lzm_chatServerEvaluation.getConfigValue('gl_default_language',false);
    if(defLang == null)
        defLang = 'en';
    ...
    var defTizo = lzm_chatServerEvaluation.getConfigValue('gl_tizo',false);
    if(defTizo == null)
        defTizo = '';

    var defshm = lzm_chatServerEvaluation.getConfigValue('gl_caen',false);
    ...
    var defMask = lzm_chatServerEvaluation.getConfigValue('gl_miat',false);
    ...
    this.m_Settings = [
        ...
        {name:'VisitorMonitoring', title: tid('monitoring'), icon:'search',groups: [
            {name: 'Monitoring', title: tid('monitoring'), controls: [
                ...
                {name:'gl_inti', standard: true, type:'int', value: 300, title: '',titleright:tid('seconds'), left:'single', top:'half'},
        ...
        {name:'Mobile', title: tid('mobile'),icon:'mobile-phone', iconcss:'margin-left:5px;margin-top:-5px;font-size:60px',groups: [
            {name: 'MobileApp', title: 'LiveZilla Mobile APPs', controls: [
...
ServerConfiguration.prototype.ApplyConfig = function() {
    ...
    for (var i=0; i<lzm_chatServerEvaluation.global_configuration.site[0].length; i++){
        var prop = lzm_displayLayout.getSettingsProperty(this.m_Settings,lzm_chatServerEvaluation.global_configuration.site[0][i].key);
        if(prop){
            if(prop.type == 'bool')
                prop.value = (lzm_chatServerEvaluation.global_configuration.site[0][i].value==1);
            else if(prop.type == 'password' || prop.type == 'string' || prop.type == 'int')
                prop.value = lzm_chatServerEvaluation.global_configuration.site[0][i].value.trim();
        }
    }

    for (var i=0; i<lzm_chatServerEvaluation.global_configuration.toplevel.length; i++){
        var prop = lzm_displayLayout.getSettingsProperty(this.m_Settings,lzm_chatServerEvaluation.global_configuration.toplevel[i].key);
        if(prop){
            if(prop.type == 'bool')
                prop.value = (lzm_chatServerEvaluation.global_configuration.toplevel[i].value==1);
            else if(prop.type == 'string' || prop.type == 'int')
                prop.value = lzm_chatServerEvaluation.global_configuration.toplevel[i].value.trim();
        }
    }
}
...
ServerConfiguration.prototype.UpdateLicenseInfo = function() {
    ...
    var trialDays = 0,licenseCountNumber = 0,html = '',breakIt = false;
    var sid = lzm_chatServerEvaluation.getConfigValue('gl_lzid',true).value;
    var created = lzm_chatServerEvaluation.getConfigValue('gl_pr_cr',true).value;
    ...
    $("#sc-license-list tbody tr").each(function() {
        if(!breakIt && $(this).attr("data-type-key")=='OPR')
        {
            ...
            amount = lzm_chatServerEvaluation.getAmount(hash,sid,serial,CommonConfigClass.lz_major);
            tdays = lzm_chatServerEvaluation.getTDays(hash,sid);
    ...
    this.m_crc3 += '1,';
    this.m_crc3 += ((trialDays > 0) ? '0' : ((licenseCountNumber != 0) ? licenseCountNumber : '-2')) +',';

    if(isNGL)
        this.m_crc3 += nglHash;

    if(trialDays > 0)
        html = '<span class="text-orange text-xl">'+tid('trial_info').replace('<!--d-->',trialDays)+'</span>';
...
ServerConfiguration.prototype.GetCustomForm = function(formType, obj) {
    ...
    if(formType== 'EmailAccounts'){
        ...
        buttonline = '<hr><div style="text-align:right;padding:8px 0;">' + lzm_inputControls.createButton('email-acc-<!--type-->-add','email_dialog_button','emailAccountAction(\'add\', \'<!--type-->\');', tid('add'), '', '', {}, '', 30, 'd') + '</div>';
        var ea_out='', ea_in='';
        for(key in lzm_chatServerEvaluation.global_configuration.database['email']){
            var ea = lzm_chatServerEvaluation.global_configuration.database['email'][key];
        ...
        for (var aChannel=0; aChannel<ChatTicketClass.m_TicketChannels.length; aChannel++) {
            ...
            {
                tsd = lzm_chatServerEvaluation.global_configuration.database['tsd'][key];
        ...
        var licNumb = 1;
        var licenseframe = lzm_chatServerEvaluation.getConfigValue('gl_licl',true);
        ...
        if(licenseframe != null)
            for (var subkey in licenseframe.subkeys)
            {
                ...
                var amount = lzm_chatServerEvaluation.getAmount(lobj[0],sid,lobj[1],CommonConfigClass.lz_major);
                var tdays = lzm_chatServerEvaluation.getTDays(lobj[0],sid);
        ...
        var nbllicense = lzm_chatServerEvaluation.getConfigValue('gl_pr_nbl',true);
        var csplicense = lzm_chatServerEvaluation.getConfigValue('gl_pr_csp',true);
        var ngllicense = lzm_chatServerEvaluation.getConfigValue('gl_pr_ngl',true);

        if(nbllicense != null)
            contentHtml += this.GetLicenseRow(licNumb++,1,'Backlink Removal','NBL','-',nbllicense.value, true, true);

        if(ngllicense != null)
            contentHtml += this.GetLicenseRow(licNumb++,1,'Geo Tracking','NGL','-',ngllicense.value, false, true);

        if(csplicense != null)
            contentHtml += this.GetLicenseRow(licNumb++,1,'Startpage','CSP','-',csplicense.value, false, true);
        ...
        var inpList = (typeof obj != 'undefined') ? obj : lzm_chatServerEvaluation.inputList.objects;
        for (var key in inpList)
        {
            var obj = inpList[key];
            var rowHtml = lzm_chatServerEvaluation.inputList.getInputHtmlRow(obj);
            if(parseInt(key)>=100)
                inps += rowHtml
            else
                inpc += rowHtml;
        ...
        contentHtml += '<tr><td>MySQL Engine</td><td>'+lzm_chatServerEvaluation.getConfigValue('gl_db_eng',false)+'</td></tr>';
        contentHtml += '<tr><td>MySQL Extension</td><td>'+lzm_chatServerEvaluation.getConfigValue('gl_db_ext',false)+'</td></tr>';
        contentHtml += '<tr><td>MySQL Host</td><td>'+lzm_chatServerEvaluation.getConfigValue('gl_db_host',false)+'</td></tr>';
        contentHtml += '<tr><td>MySQL User</td><td>*********</td></tr>';
        contentHtml += '<tr><td>MySQL Password</td><td>*********</td></tr>';
        contentHtml += '<tr><td>MySQL Database</td><td>'+lzm_chatServerEvaluation.getConfigValue('gl_db_name',false)+'</td></tr>';
        contentHtml += '<tr><td>MySQL Prefix</td><td>'+lzm_chatServerEvaluation.getConfigValue('gl_db_prefix',false)+'</td></tr>';
        contentHtml += '</tbody></table>';
    }
        ...
        for(key in lzm_chatServerEvaluation.global_configuration.database['fbc'])
            contentHtml += this.GetCriteriaRow(lzm_chatServerEvaluation.global_configuration.database['fbc'][key]);
        ...
        var invi = lzm_commonTools.phpUnserialize(lz_global_base64_decode(lzm_chatServerEvaluation.getConfigValue('gl_invi',false).toString()));
        contentHtml = '<table class="spaced" style="max-width:1000px;"><tr><td colspan="3"><fieldset class="lzm-fieldset-full" style="margin-top:-10px;height:300px;"><legend>'+tid('general')+'</legend>';
        ...
        contentHtml += '<div class="top-space">' + lzm_inputControls.createInput('gl_caii','',lzm_chatServerEvaluation.getConfigValue('gl_caii',false),tidc('company_logo') + ' (URL)','','text','','','')+ '</div>';
        ...
        contentHtml += '</fieldset></td></tr><td><fieldset class="lzm-fieldset-full" style="margin-top:-10px;height:260px;"><legend>'+tid('margin')+'</legend>';
        ...
        contentHtml += '</fieldset></td><td><fieldset class="lzm-fieldset-full" style="margin-top:-10px;height:260px;"><legend>'+tid('shadow')+'</legend>';
        ...
        contentHtml += '</fieldset></td><td><fieldset class="lzm-fieldset-full" style="margin-top:-10px;height:260px;"><legend>'+tid('background')+'</legend>';
        ...
        contentHtml += lzm_inputControls.createInput('gl_cali','',lzm_chatServerEvaluation.getConfigValue('gl_cali',false),'','','text','','','','URL: ');
        ...
        contentHtml += lzm_inputControls.createButton('sc-header-image-default','','', tid('default'), '', '', {'margin-top':'4px'}, '', 30, 'd');
        contentHtml += '</td></tr></table>';
    }
    else if(formType == 'BackgroundImage'){
        contentHtml = '<div id="sc-background-image" class="bottom-space"></div>';
        contentHtml += '<table><tr><td>';
        contentHtml += lzm_inputControls.createInput('gl_cahi','',lzm_chatServerEvaluation.getConfigValue('gl_cahi',false),'','','text','','','','URL: ');
        contentHtml += '</td><td style="text-align:right">';
        contentHtml += lzm_inputControls.createButton('sc-background-image-preview','','', tid('preview'), '', '', {'margin-right':'6px'}, '', 30, 'd');
        contentHtml += lzm_inputControls.createButton('sc-background-image-default','','', tid('default'), '', '', {'margin-top':'4px'}, '', 30, 'd');
        contentHtml += '</td></tr></table>';
    }
    return contentHtml;
}
...
ServerConfiguration.prototype.GetLicenseRow = function(number, amount, typeName, typeKey, serial, hash, conf, deac) {
    ...
    if(deac)
        return contentHtml + '&nbsp;' + lzm_inputControls.createButton('sc-deac-lic-'+hash,'','deactivateLicense(\''+hash+'\');', tid('deactivate'), '', '', {}, '', '')+'</td></tr>';
    else
...
ServerConfiguration.prototype.GetEmailAccountRow = function(accountObj) {
    ...
    var buttonrow =
        lzm_inputControls.createButton('email-acc-'+accountObj.id+'-edit','email_dialog_button','emailAccountAction(\'edit\',\''+accountObj.boxtype+'\',\''+accountObj.id+'\');', tid('edit'), '<i class="fa fa-cog icon-small"></i>', '', {'margin': '0 6px 0 0'}, '', 30, 'b') +
        lzm_inputControls.createButton('email-acc-'+accountObj.id+'-remove','email_dialog_button','emailAccountAction(\'remove\',\''+accountObj.boxtype+'\',\''+accountObj.id+'\');', tid('remove'), '<i class="fa fa-trash icon-small"></i>', '', {'margin': '0 6px 0 0'}, '', 30, 'b') +
        lzm_inputControls.createButton('email-acc-'+accountObj.id+'-test','email_dialog_button','emailAccountAction(\'test\',\''+accountObj.boxtype+'\',\''+accountObj.id+'\');', tid('test'), '<i class="fa fa-bolt icon-small"></i>', '', {'margin': '0'}, '', 30, 'b');
...
ServerConfiguration.prototype.ApplyLogicToForm = function() {
    ...
    if(lzm_chatServerEvaluation.getConfigValue('gl_alloc_mode',false) == '1')
        $('#r-gl_alloc_mode_1').prop('checked',true);

    if(lzm_chatServerEvaluation.getConfigValue('gl_mcwf',false) != null)
        $('#r-gl_mcwf_'+lzm_chatServerEvaluation.getConfigValue('gl_mcwf',false).toString()).prop('checked',true);
    ...
    if(lzm_chatServerEvaluation.getConfigValue('gl_om_mode',false) == '1')
    ...
    if(lzm_chatServerEvaluation.getConfigValue('gl_ldap',false) != null)
        $('#r-gl_ldap_'+lzm_chatServerEvaluation.getConfigValue('gl_ldap',false).toString()).prop('checked',true);
    $('.auth-select').change();
    ...
    // https
    if(lzm_chatServerEvaluation.getConfigValue('gl_rhts',false) == '1')
    ...
    if(lzm_chatServerEvaluation.getConfigValue('gl_imda',false) == '1')
        $('#r-gl_imda_1').prop('checked',true);
    else
        $('#r-gl_imda_0').prop('checked',true);

    if(lzm_chatServerEvaluation.getConfigValue('gl_iada',false) == '1')
    ...
    if(lzm_chatServerEvaluation.getConfigValue('gl_kbin',false) == '1')
        $('#r-gl_kbin_1').prop('checked',true);
    else if(lzm_chatServerEvaluation.getConfigValue('gl_kbin',false) == '2')
    ...
    if(lzm_chatServerEvaluation.getConfigValue('gl_kbsb',false) == '1')
        $('#r-gl_kbsb_1').prop('checked',true);
    else if(lzm_chatServerEvaluation.getConfigValue('gl_kbsb',false) == '2')
        $('#r-gl_kbsb_2').prop('checked',true);
    else
        $('#r-gl_kbsb_0').prop('checked',true);

    if(lzm_chatServerEvaluation.getConfigValue('gl_bldo',false) == '1')
    ...
    $('#sc-header-image-default').click(function(){
        $('#gl_cali').val(lzm_chatServerEvaluation.getServerUrl('') + 'images/carrier_logo.png');
    ...
    $('#sc-background-image-default').click(function(){
        $('#gl_cahi').val(lzm_chatServerEvaluation.getServerUrl('') + 'images/carrier_header.gif');
