        ...
        for (i=0; i<webSites.length; i++) {
            ...
            if (replaceLink)
            {
                if (webSites[i].toLowerCase().indexOf('http') != 0)
                    replacement = '<a target="_blank" class="lz_chat_link" href="http://' + webSites[i] + '" data-url="http://' + webSites[i] + '">' + lzm_commonTools.SubStr(webSites[i],30,true) + '</a>';
                else
                    replacement = '<a target="_blank" class="lz_chat_link" href="' + webSites[i] + '" data-url="' + webSites[i] + '">' + lzm_commonTools.SubStr(webSites[i],30,true) + '</a>';
...
CommonToolsClass.prototype.replaceChatPlaceholders = function(reco, resourceText) {
    ...
    var replacementArray = [
        ...
        {p: '%page_title%', r: pageTitle}, {p: '%url%', r: url}, {p: '%domain%', r: domain}, {p: '%searchstring%', r: searchString},
...
CommonToolsClass.prototype.guid = function() {
    return md5(Math.floor((1 + Math.random()) * 0x10000).toString());
};
