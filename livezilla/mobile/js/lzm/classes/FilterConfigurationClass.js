...
FilterConfiguration.prototype.showFilterCreation = function(type, visitor, filter, ticket, inDialog) {
    ...
    $('#save-filter').click(function() {
        ...
        $('#filter-list-loading').css({position: 'absolute', left: '0px', top: '0px', bottom: 0, right:0,'background-color': '#ffffff', 'z-index': 1000});
    });
};
...
FilterConfiguration.prototype.createVisitorFilterMainHtml = function(type, visitor, filter, ticket) {
    ...
    if(type =='visitor')
        filterHtml+= '<fieldset style="margin-top: 10px;" class="lzm-fieldset" data-role="none" id="visitor-filter-base">' +
            ...
            '<td colspan="2">' +
            '<span><input disabled type="text" data-role="none" class="lzm-filter-input-main' + visitorIdDisabled + '" id="filter-id" value="' + visitorId + '" /></span></td>' +
            '</tr><tr class="top-space-half"><td>' +
...
FilterConfiguration.prototype.createFilterListHtml = function() {
    ...
    for (var i=0; i<filterList.length; i++) {
        ...
        var onclickAction = (!lzm_chatDisplay.isApp && !lzm_chatDisplay.isMobile) ? ' onclick="selectFiltersLine(event, \'' + filterList[i].filterid + '\');"' : ' onclick="openFiltersListContextMenu(event, \'' + filterList[i].filterid + '\');"';
        var onconetxtMenuAction = (!lzm_chatDisplay.isApp && !lzm_chatDisplay.isMobile) ? ' oncontextmenu="openFiltersListContextMenu(event, \'' + filterList[i].filterid + '\');"' : '';
        var ondblclickAction =  (!lzm_chatDisplay.isApp && !lzm_chatDisplay.isMobile) ? ' ondblclick="showFilterCreation(\'edit\',\'\', \'\', \'' + filterList[i].filterid + '\', true);"' : '';
