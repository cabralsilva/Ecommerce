...
AdminUserManagementClass.prototype.createOperatorList = function() {
    ...
    for (var i=0; i<operators.length; i++) {
        ...
        icon = '<div class="avatar-box avatar-box-small avatar-box-'+iconcol+'" style="background-image: url(./../picture.php?intid='+lz_global_base64_url_encode(operators[i].id)+'"></div>';
...
AdminUserManagementClass.prototype.createBotConfiguration = function(bot) {
    ...
    var tabList = [
        ...
        {name: t('Picture'), content: that.createPictureConfiguration(bot,true), hash: 'picture'},
...
AdminUserManagementClass.prototype.LDAPSearch = function(type) {
    if(type == 'user')
    {
        var ldapHtml = '<div id="ldap-search">'
            +'<fieldset class="lzm-fieldset-full"><legend>LDAP</legend><table class="tight"><tr><td>'+lzm_inputControls.createInput('search-ldap-sname','ui-disabled',parent.lzm_chatServerEvaluation.getConfigValue('gl_ldho',false),'','','text','')+'</td><td>'
        ...
        $('#search-ldap-btn',parent.document).click(function() {
            ...
            parent.lzm_chatPollServer.pollServerDiscrete('ldap_search',data).done(function(data)
            {
...
AdminUserManagementClass.prototype.createOperatorConfiguration = function(operator) {
    ...
    var tabList = [
        ...
        {name: t('Picture'), content: that.createPictureConfiguration(operator,false), hash: 'picture'},
...
AdminUserManagementClass.prototype.saveUser = function() {
    ...
    $('#operator-uid').val($('#operator-uid').val().replace(/[_\W]+/g, ""));
...
AdminUserManagementClass.prototype.createPictureConfiguration = function(operator,isBot) {
    ...
    var contentHtml = '<fieldset class="lzm-fieldset op-config-fs" id="op-picture-configuration"><legend>' + t('Picture') + '</legend>' + opPicture +
        ...
        lzm_inputControls.createInput('operator-pic', 'umg-edit-text-input', '', tidc('from_file'), '<i class="fa fa-file-o"></i>', 'file', 'a') +
        '<input type="hidden" value="' + pp + '" id="operator-pic-b64" />' +
        '<div class="lzm-info-text top-space-half">' + t('Aspect ratio 4:3') + '</div>' +
        '</div></fieldset>'+
        '<fieldset class="lzm-fieldset" style="display:'+pVisible+'"><legend>' + t('Permissions') + '</legend>'+
    ...
    var opList = that.operators.getOperatorList();
        for (var i=0; i<opList.length; i++) {
            if (opList[i].isbot != 1 && (operator == null || operator.id != opList[i].id)) {
                ...
                contentHtml += '<tr><td style="width: 30px !important;text-align:center;"><input type="checkbox" class="checkbox-custom" class="op-pref-account" data-op-id="' + opList[i].id + '"' +
                    ' id="op-pref-account-' + opList[i].id + '"' + opIsPreffered + ' /><label class="checkbox-custom-label" for="op-pref-account-' + opList[i].id + '"></td>' +
...
AdminUserManagementClass.prototype.createGroupTicketsConfiguration = function(group, selectedTab) {
    ...
    var emailTabContent = '<fieldset class="lzm-fieldset" id="gr-tickets-email-out-fs"><legend>' + t('Outgoing Emails') + '</legend>' +
        '<div><input type="radio" name="gr-tickets-out-mta" class="gr-tickets-out-mta-radio radio-custom" id="gr-tickets-out-client"' + clientChecked + ' />' +
        '<label for="gr-tickets-out-client" class="radio-custom-label">' + t('Operators will send emails through the local email client (Outlook, Thunderbird etc)') + '</label></div>' +
        '<div class="top-space-half"><input type="radio" name="gr-tickets-out-mta" class="gr-tickets-out-mta-radio radio-custom" id="gr-tickets-out-lz"' + lzChecked + ' />' +
        '<label for="gr-tickets-out-lz" class="radio-custom-label">' + t('Operators will send emails through LiveZilla') + '</label></div>' +
...
AdminUserManagementClass.prototype.createSocialMediaChannel = function(smc) {
    ...
    var useTrack = (smc != null && typeof smc.tr != 'undefined' && smc.tr != '');
    var trackDisabled = (smc != null && typeof smc.tr != 'undefined' && smc.tr != '') ? '' : 'ui-disabled';
    var useMention = true;
    var mentionDisabled = 'ui-disabled';
    ...
    if (smc != null && smc.t == 7) {
        inputForm += '<fieldset class="lzm-fieldset"><legend>' + tid('tweets') + '</legend>' +
            '<div>' + lzm_inputControls.createCheckbox('smc-use-me',tid('create_tickets_tweets_me'),useMention,'',mentionDisabled) + '</div>' +
            '<div class="top-space-half">' + lzm_inputControls.createCheckbox('smc-use-track',tid('create_tickets_tweets_hashtag'),useTrack,'',trackDisabled) + '</div>' +
            '<div class="top-space-half left-space-child">' + lzm_inputControls.createInput('smc-track', trackDisabled, smcTrack, tidc('hashtags'), '', 'text', '') + '</div>' +
            '</fieldset>';
    }
    ...
    $('#smc-use-track').change(function(){
       if($('#smc-use-track').prop('checked'))
           $('#smc-track').removeClass('ui-disabled');
       else
           $('#smc-track').addClass('ui-disabled');
    });
...
AdminUserManagementClass.prototype.createNewObjectMenu = function() {
    ...
    disabled = (parent.lzm_chatServerEvaluation.getConfigValue('gl_ldap',false)!='1') ? ' class="ui-disabled"' : '';
