...
CommonPermissionClass.prototype.getUserPermissions = function(initial, operatorId, permissions) {
    operatorId = (typeof operatorId != 'undefined') ? operatorId : (typeof lzm_chatServerEvaluation != 'undefined') ? lzm_chatServerEvaluation.myId : '';
    permissions = (typeof permissions != 'undefined') ? permissions : [];
    var i = 0;
    if (initial && typeof lzm_chatServerEvaluation != 'undefined') {
        permissions = lzm_chatServerEvaluation.permissions
    } else if (permissions.length == 0) {
        var operator = (typeof lzm_chatServerEvaluation != 'undefined') ? lzm_chatServerEvaluation.operators.getOperator(operatorId) :
...
CommonPermissionClass.prototype.checkUserResourceWritePermissions = function(uid, action, resource) {
    var rtValue = false;
    var parent = lzm_chatServerEvaluation.cannedResources.getResource(resource.pid);
    ...
    switch(action) {
        case 'add':
            ...
            if(this.permissions.resources_write == 2){
                ...
                if ($.inArray(resource.g, lzm_chatServerEvaluation.operators.getOperator(uid).groups) != -1)return true;
                if (resource.g == '' && $.inArray(parent.g, lzm_chatServerEvaluation.operators.getOperator(uid).groups) != -1)return true;
            }
            if(this.permissions.resources_write == 1){
                if (resource.oid == uid && resource.ty == 0)return true;
                if (parent.oid == uid && resource.ty != 0)return true;
            }
            break;
        case 'delete':
        case 'edit':
...
CommonPermissionClass.prototype.checkUserResourceReadPermission = function(uid, resource, parent) {
    ...
    {
        if (resource.ty != 0 && $.inArray(parent.g, lzm_chatServerEvaluation.operators.getOperator(uid).groups) == -1)
            return false;

        if (resource.ty == 0 && $.inArray(resource.g, lzm_chatServerEvaluation.operators.getOperator(uid).groups) == -1)
            return false;
    }
...
CommonPermissionClass.prototype.checkUserTicketPermissions = function(uid, action, ticket) {
    ...
    switch(action) {
        case 'view':
            var myGroups = [];
            var operator = lzm_chatServerEvaluation.operators.getOperator(uid);
            if (operator != null) {
