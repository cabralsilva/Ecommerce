...
ChatTicketClass.prototype.createTicketListHtml = function(tickets, ticketGlobalValues, page, sort, sortDir, query, filter, elementId) {
    ...
    var ww=$(window).width(),fullScreenMode = this.isFullscreenMode(), that = this, i, emailDisabledClass = (ticketGlobalValues['e'] > 0) ? 'lzm-button-b-active' : 'ui-disabled';
    var createDisabledClass = '', totalTickets = ticketGlobalValues.t;
    ...
    if ($(window).width() > 800) {
        var ticketListInfo = (lzm_chatPollServer.ticketFilterStatus.length == 4 && lzm_chatPollServer.ticketQuery == '') ? ticketListInfo2 : ticketListInfo1;
    ...
    ticketListHtml += '<span class="right-button-list" style="margin-right:129px;">' +
        lzm_inputControls.createButton('ticket-show-emails', emailDisabledClass, '', t('Emails <!--number_of_emails-->',[['<!--number_of_emails-->', '(' + ticketGlobalValues['e'] + ')']]), '<i class="fa fa-envelope-o"></i>', 'lr',{'margin-right': '4px'}, '', 20) +
        lzm_inputControls.createButton('ticket-create-new', createDisabledClass, '', t('Create Ticket'), '<i class="fa fa-plus"></i>', 'lr', {'margin-right': '8px'}, '', 20) + '</span>' +
        lzm_inputControls.createInput('search-ticket','', query, t('Search'), '<i class="fa fa-remove"></i>', 'text', 'b') + '</div>';


    var ticketListBodyCss = ((lzm_chatDisplay.isApp || lzm_chatDisplay.isMobile) || ww <= 1000) ? ' style="overflow: auto;"' : '';
    ...
    if(fullScreenMode)
    {
        ...
        for (i=0; i<lzm_chatServerEvaluation.inputList.idList.length; i++) {
            ...
            if (myCustomInput.active == 1 && parseInt(myCustomInput.id) < 111 && myCustomInput.display.ticket) {
                var inputText = '';
                inputText = (inputText != '') ? inputText : '-';
                ticketListHtml += '<th>' + myCustomInput.name + '</th>';
            }
        }
    }
...
ChatTicketClass.prototype.createTicketListLine = function(ticket, lineCounter, inDialog, elementId, fullScreenMode) {
    ...
    var dueTimeFull = lzm_chatServerEvaluation.getConfigValue('gl_tidt',false)*3600;
    var dueTimeHalf = lzm_chatServerEvaluation.getConfigValue('gl_tidt',false)*2700;
    ...
    if(fullScreenMode)
    {
        ...
        for (i=0; i<lzm_chatServerEvaluation.inputList.idList.length; i++) {
            ...
            if (myCustomInput.active == 1 && parseInt(myCustomInput.id) < 111 && myCustomInput.display.ticket) {
                var inputText = '';
                for (var j=0; j<ticket.messages[0].customInput.length; j++)
                    if (ticket.messages[0].customInput[j].id == myCustomInput.name)
                        inputText = (myCustomInput.type != 'CheckBox') ? lzm_commonTools.htmlEntities(ticket.messages[0].customInput[j].text) : (ticket.messages[0].customInput[j].text == 1) ? t('Yes') : t('No');
                    inputText = (inputText != '') ? inputText : '-';
                addClass = getWaitingTimeClass(waitingTime);
                inputText = (columnContents[j].contents != '') ? inputText : '-';
    ...
    else
    {
        ...
        svContent += '<div class="lzm-info-text">' + lzm_commonTools.htmlEntities(lzm_commonTools.SubStr(ticket.messages[0].mt,200,true)) + '</div>';
...
ChatTicketClass.prototype.createMatchingTickets = function(elementId) {
    elementId = (typeof elementId != 'undefined') ? elementId : '';
    var tickets = [],that = this,matchingTicketsHtml = '<div data-role="none" id="matching-tickets-'+elementId+'-inner">' +
        '<table id="matching-tickets-'+elementId+'-table" class="visible-list-table alternating-rows-table lzm-unselectable" style="width: 100%;margin-top:1px;">' + that.createMatchingTicketsTableContent(tickets,elementId) + '</table></div>';
...
ChatTicketClass.prototype.createMatchingTicketsTableContent = function(tickets, elementId) {
    ...
    for (i=0; i<lzm_chatServerEvaluation.inputList.idList.length; i++) {
        ...
        if (myCustomInput.active == 1 && parseInt(myCustomInput.id) < 111 && myCustomInput.display.ticket) {
            var inputText = '';
            inputText = (inputText != '') ? inputText : '-';
            tableHtml += '<th>' + myCustomInput.name + '</th>';
        }
    }
...
ChatTicketClass.prototype.updateTicketDetails = function(selectedTicket) {
    ...
    var commentsHtml = '' + that.createTicketCommentTable(selectedTicket, selectedMessage, '', 'ticket-details-placeholder-tab-2');
...
ChatTicketClass.prototype.showTicketDetails = function(ticket, isNew, email, chat, existingDialogId) {
    ...
    var footerString = '<span style="float: left;">' +
        lzm_inputControls.createButton('reply-ticket-details', 'ticket-buttons' + disabledButtonClass, '', t('Reply'), '<i class="fa fa-mail-reply"></i>', 'lr', {'margin-left': '6px'}, '', 20, 'e') +
    ...
    var commentsHtml = '<div id="ticket-comment-list" class="lzm-frameset" data-role="none">' + that.createTicketCommentTable(ticket, lastMessage, menuEntry, 'ticket-details-placeholder-tab-2') + '</div>';
    ...
    lzm_displayHelper.createTabControl('ticket-details-placeholder', [
        ...
        {name: tid('comments'), content: commentsHtml},
    ...
    $('#add-attachment').click(function(){
        ...
        {
            lzm_displayHelper.minimizeDialogWindow(dialogId, 'ticket-details',{'ticket-id': -1, menu: menuEntry}, 'tickets', false);
            lzm_chatUserActions.addQrd(3,'', true, dialogId, null, menuEntry);
        }
        else
    ...
    $('#apply-ticket-details').click(function() {
        ...
        else
        {
            ...
            if($('#ticket-details-priority').prop('selectedIndex').toString() != ticket.p)
            ...
            if (/*existingDialogId == '' && */email.id == '' && chat.cid == '') {
                ...
                if ($('#message-details-inner').data('edit')) {
                    ...
                    for (i=0; i<lzm_chatServerEvaluation.inputList.idList.length; i++) {
                        ...
                        var myCustomInputValue = lzm_chatServerEvaluation.inputList.getControlValue(myCustomInput,'change-message-custom-' + myCustomInput.id);
                ...
                initSaveTicketDetails(ticket, $('#ticket-details-channel').val(), $('#ticket-details-status').val(),
                    $('#ticket-details-group').val(), $('#ticket-details-editor').val(), $('#ticket-details-language').val(),
                    $('#ticket-new-name').val(), $('#ticket-new-email').val(), $('#ticket-new-company').val(), $('#ticket-new-phone').val(),
                    $('#ticket-new-input').val(), attachments, comments, customFields,$('#ticket-details-sub-status').val(),$('#ticket-details-sub-channel').val(),{cid: ''},'',$('#ticket-new-subject').val(),rem_time,rem_status);
...
ChatTicketClass.prototype.readCustomFields = function(){
    ...
    for (var i=0; i<lzm_chatServerEvaluation.inputList.idList.length; i++) {
        ...
        if (myCustomInput.active == 1 && parseInt(myCustomInput.id) < 111)
            customFields[myCustomInput.id] = lzm_chatServerEvaluation.inputList.getControlValue(myCustomInput,'ticket-new-cf' + myCustomInput.id);
    }
    return customFields;
};
...
ChatTicketClass.prototype.getReminderHtml = function(ticket){
    ...
    var status = '1';
    var span =  lzm_commonStorage.loadValue('ticket_reminder_span_' + lzm_chatServerEvaluation.myId);
    if(span == null)
        span = 604800;
...
ChatTicketClass.prototype.createTicketDetails = function(ticketId, ticket, email, chat, disabledString, isNew, selectedGroup) {
    var selectedString, selectedLanguage = '', availableLanguages = [], disabledClass ='';
    ...
    for (var aChannel=0; aChannel<ChatTicketClass.m_TicketChannels.length; aChannel++) {
        ...
        selectedString = (channel.index == ticket.t || (email.id != '' && channel.key == 'chat')) ? ' selected="selected"' : (chat.cid != '' && channel.index == 4) ? ' selected="selected"' : '';
...
ChatTicketClass.prototype.createTicketCommentTable = function(ticket, messageNumber, menuEntry, tabName) {
    ...
    var commentsHtml = '<div id="ticket-comment-list-header" class="lzm-dialog-headline3">' +
        lzm_inputControls.createButton('add-comment', '', 'addComment(\'' + ticket.id + '\', \'' + menuEntry + '\')', t('Add Comment'), '<i class="fa fa-plus"></i>', 'lr', {float:'right','margin-right':'4px'}, t('Add Comment')) +

    '</div><div id="ticket-comment-list-frame" style="overflow-y:auto;"><table id="comment-table" class="visible-list-table alternating-rows-table lzm-unselectable" style="width: 100%">' +
        '<tbody>';
    if (typeof ticket.messages != 'undefined' && typeof ticket.messages[messageNumber] != 'undefined') {
        for (k=0; k<ticket.messages[messageNumber].comment.length; k++) {
...
ChatTicketClass.prototype.showMessageReply = function(ticket, messageNo, selectedGroup, menuEntry) {
    ...
    $('#add-attachment').click(function()
    {
        if (!lzm_chatDisplay.isApp && !lzm_chatDisplay.isMobile) {
            ...
            lzm_chatUserActions.addQrd(3,'', true, myDialogId, null, menuEntry);
        } else
            showNotMobileMessage();
    });
    $('#add-attachment-from-qrd').click(function() {
...
ChatTicketClass.prototype.showMessageReplyPreview = function(ticket, messageNo, message, signature, comment, attachments, salutation, group, menuEntry, answerInline) {
    menuEntry = (typeof menuEntry != 'undefined') ? menuEntry : '';
    var that = this, messageId = md5(Math.random().toString());
    var email = '', bcc = '', i = 0, subjObject = {}, defLanguage = 'EN';
    ...
    if (ticket.t != 6 && ticket.t != 7) {
        previewHtml += '<label class="top-space" for="ticket-reply-subject">' + tidc('subject') + '</label>' +
        ...
        for (var i=ticket.messages.length - 1; i>=0; i--) {
            ...
            else
            {
                ...
                messageListHtml += '<tr class="message-line message-line-fs lzm-unselectable" id="message-line-' + ticket.id + '_' + i + '"' +
                    onclickAction + oncontextMenu + ondblclickAction + '>' +
                    '<td style="width:60px;background:'+linecol+';">' +
                    '<div class="avatar-box" style="background-image: url(./../picture.php?intid='+lz_global_base64_url_encode(ticket.messages[i].sid)+'"></div>' +
...
ChatTicketClass.prototype.createTicketMessageAddLine = function(type,key,mkey,ticket,object,linecol,onclickAction,oncontextAction) {
    ...
    if(type=="comment")
    {
        var coperator = lzm_chatServerEvaluation.operators.getOperator(object.o);
        lineHtml += '<tr class="message-line message-comment-line comment-line-' + ticket.id + '_' + mkey + ' lzm-unselectable" id="message-line-comment-' + ticket.id + '_' + key + '" '+onclickAction+oncontextAction+'>' +
            ...
            '<span class="text-gray nic">' + lzm_commonTools.htmlEntities(coperator.name) + ': </span><span class="text-gray nic" style="white-space:normal;font-style: italic;">' + lzm_commonTools.htmlEntities(object.text) + '</span>' +
            '</div></td></tr>';
    }
    else if(type=="log")
    {
        ...
        var messageTimeHuman = lzm_commonTools.getHumanDate(messageTimeObject, '', lzm_chatDisplay.userLanguage);
        var loperator = lzm_chatServerEvaluation.operators.getOperator(object.o);
...
ChatTicketClass.prototype.updateEmailList = function() {
    ...
    else
    {
        ...
        $('#cancel-email-list').click();
        $('#ticket-show-emails').addClass('ui-disabled');
        return;
    }
...
ChatTicketClass.prototype.createTicketListContextMenu = function(myObject, place, widthOffset) {
    ...
    for(key in lzm_chatServerEvaluation.global_configuration.database['tsd'])
    {
        elem = lzm_chatServerEvaluation.global_configuration.database['tsd'][key];
...
ChatTicketClass.prototype.subDefinitionIsValid = function(type,parent,sub){
    for(var key in lzm_chatServerEvaluation.global_configuration.database['tsd'])
    {
        var tsd = lzm_chatServerEvaluation.global_configuration.database['tsd'][key];
...
ChatTicketClass.prototype.setTicketFilter = function(){
    ...
    for (i=0; i<groups.length; i++)
    {
        check = (lzm_chatPollServer.ticketFilterGroups != null) ? lzm_chatPollServer.ticketFilterGroups.indexOf('\'' + groups[i].id + '\'')!==-1 : true;
    ...
    for (var aChannel=0; aChannel<ChatTicketClass.m_TicketChannels.length; aChannel++) {
        ...
        check = (lzm_chatPollServer.ticketFilterChannel != null) ? lzm_chatPollServer.ticketFilterChannel.toString().indexOf(aChannel.toString())!==-1 : true;
        ...
        for(key in lzm_chatServerEvaluation.global_configuration.database['tsd'])
        {
            ...
            if(tsd.type == 1 && tsd.parent == sc.index){
                check = (lzm_chatPollServer.ticketFilterSubChannels != null) ? lzm_chatPollServer.ticketFilterSubChannels.indexOf(lz_global_base64_encode(tsd.parent + tsd.name))!==-1 : true;
    ...
    $('#stf-ok-btn').click(function() {
        ...
        lzm_chatPollServer.ticketFilterGroups = (allChecked) ? null : fgroups;

        if(!allChecked)
            lzm_commonStorage.saveValue('ticket_filter_groups_' + lzm_chatServerEvaluation.myId,lz_global_base64_encode(fgroups));
        else
            lzm_commonStorage.deleteKeyValuePair('ticket_filter_groups_' + lzm_chatServerEvaluation.myId);
        ...
        lzm_chatPollServer.ticketFilterChannel = (allChannelsChecked) ? '01234567' : fchannels;
        lzm_chatPollServer.ticketFilterSubChannels = (allSubChannelsChecked) ? null : fsubchannels;

        if(!allChannelsChecked)
            lzm_commonStorage.saveValue('ticket_filter_channel_' + lzm_chatServerEvaluation.myId,fchannels);
        else
            lzm_commonStorage.deleteKeyValuePair('ticket_filter_channel_' + lzm_chatServerEvaluation.myId);

        if(!allSubChannelsChecked)
            lzm_commonStorage.saveValue('ticket_filter_sub_channels_' + lzm_chatServerEvaluation.myId,fsubchannels);
        else
            lzm_commonStorage.deleteKeyValuePair('ticket_filter_sub_channels_' + lzm_chatServerEvaluation.myId);
