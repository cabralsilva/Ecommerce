...
ChatUserActionsClass.prototype.translateTextAndSend = function(visitorChat, chatMessage, chatReco) {
    if(!this.messageFromKnowledgebase || true){
...
ChatUserActionsClass.prototype.setActiveChat = function (active_chat, active_chat_reco, thisUser) {
...
ChatUserActionsClass.prototype.saveUserSettings = function(settings, multiServerId, isApp) {
    ...
    {
        var tableNames = lzm_chatDisplay.settingsDisplay.tableIds, i = 0;
...
ChatUserActionsClass.prototype.chatInternalWith = function (id, userid, name, fromOpList) {
    ...
    this.setActiveChat(id, id, thisUser);
    ...
    if (lzm_chatServerEvaluation.userChats.getUserChat(lzm_chatDisplay.active_chat) == null) {
        lzm_chatServerEvaluation.userChats.setUserChat(lzm_chatDisplay.active_chat, {status:'read', type:'internal', id: id, b_id:'', group_chat: false});
    }
...
ChatUserActionsClass.prototype.viewUserData = function (id, b_id, chat_id, freeToChat, newlyAcceptedChat) {
    ...
    var userChat = lzm_chatServerEvaluation.userChats.getUserChat(active_chat_reco);
    if(userChat != null && lzm_chatServerEvaluation.userChats.isInPublicGroupChat(userChat))
        return;
    ...
    thisClass.setActiveChat(active_chat, active_chat_reco, thisUser);
    ...
    if ($.inArray(id + '~' + b_id, thisClass.open_chats) != -1 &&  (lzm_chatServerEvaluation.userChats.getUserChat(id + '~' + b_id) == null || !lzm_chatServerEvaluation.userChats.getUserChat(id + '~' + b_id).group_chat))
    {
    ...
    else
    {
        ...
        thisClass.lzm_chatDisplay.createChatHtml(thisUser);
        ...
        $('#forward-chat').click(function () {
            ...
            if (lzm_commonPermissions.checkUserPermissions('', 'chats', 'forward', {})) {
                ...
                if (storedForwardId != '')
                    ...
                    thisClass.lzm_chatDisplay.ChatForwardInvite.createOperatorForwardInviteHtml('forward', lzm_chatPollServer.thisUser, id, b_id, chat_id);
    ...
    lzm_chatDisplay.updateChatElements(thisUser);
};
...
ChatUserActionsClass.prototype.declineChat = function(id, b_id, chat_id){
    if (lzm_commonPermissions.checkUserPermissions('', 'chats', 'decline', {})) {
        this.refuseExternalChat(id, b_id, chat_id, 0);
        this.lzm_chatDisplay.removeSoundPlayed(id + '~' + b_id);
        this.viewUserData(id, b_id, chat_id);
    } else {
        showNoPermissionMessage();
    }
}
...
ChatUserActionsClass.prototype.acceptChat = function(id, b_id, chat_id, active_chat_reco, visitorLangString, showSalutation) {
    ...
    if ($.inArray(id + '~' + b_id, thisClass.open_chats) == -1) {
        ...
        if ($.inArray(chat_id, thisClass.localAcceptedChats) == -1)
            thisClass.localAcceptedChats.push(chat_id);
        ...
        if (isNewChat)
        {
            new_chat.text = t('<!--this_op_name--> has accepted the chat.',[['<!--this_op_name-->',thisClass.lzm_chatServerEvaluation.myName]]);
        }
        else
        {
            var oldMembers = visitorBrowser[1].chat.pn.oldMemberIdList;
            visitorBrowser[1].chat.pn.acc = 1;

            if (typeof oldMembers == "object" && oldMembers instanceof Array && oldMembers.length > 0)
            {
                //new_chat.text = tid('op_joined_chat').replace('<!--this_op_name-->',lzm_chatDisplay.myName);
...
ChatUserActionsClass.prototype.chatExternalWith = function (id, b_id, chat_id, chat_no) {
    ...
    thisClass.lzm_chatDisplay.createChatHtml(lzm_chatPollServer.thisUser);
    ...
    thisInviteOperator.click(function () {
        thisClass.lzm_chatDisplay.ChatForwardInvite.createOperatorForwardInviteHtml('invite', lzm_chatPollServer.thisUser, id, b_id, chat_id);
    });
...
ChatUserActionsClass.prototype.leaveExternalChat = function (id, b_id, chat_id, chat_no, closeOrLeave) {
    ...
    new_chat.sen = '0000000';
    new_chat.rec = '';
    new_chat.reco = id +'~'+ b_id;//this.lzm_chatServerEvaluation.active_chat_reco;
    var tmpdate = lzm_chatTimeStamp.getLocalTimeObject();
    new_chat.date = lzm_chatTimeStamp.getServerTimeString(tmpdate, true);
    new_chat.cmc = lzm_chatServerEvaluation.chatMessageCounter;
    lzm_chatServerEvaluation.chatMessageCounter++;
    ...
    new_chat.text = tid('op_has_left',[['<!--this_op_name-->',this.lzm_chatServerEvaluation.myName]]);
...
ChatUserActionsClass.prototype.forwardChat = function (thisUser, type) {
    ...
    if (typeof this.forwardData.id != 'undefined') {
        ...
        this.lzm_chatDisplay.ChatForwardInvite.createOperatorForwardInviteHtml(type, thisUser);
        this.lzm_chatPollServer.stopPolling();
    ...
    if (resource != null) {
        ...
        switch(parseInt(newType)) {
            ...
            case 1: // HTML resource
                if((!this.isApp && !this.isMobile) || true) {
                    editResource.css('display', 'none');
    ...
    */
    $('#save-new-qrd').click(function() {
        ...
        if (newType != 3)
        {
            ...
            var ownerOPId = lzm_chatServerEvaluation.myId;
            var ownerGRId = '';
            ...
            if (sendToChat == null)
            {
                ...
                var newEntryHtml = '<div id="resource-' + newRid + '" class="resource-div" style="padding-left: ' + (20 * newRank) + 'px; margin: 4px 0px;">';
                if (newType == 0) {
                    newEntryHtml += '<span id="resource-' + newRid + '-open-mark" style=\'display: inline-block; width: 7px; height: 7px; border: 1px solid #aaa; background-color: #f1f1f1; ' +
                ...
                newEntryHtml += '<span style=\'background-image: url("' + lzm_chatDisplay.resourcesDisplay.getResourceIcon(newResource) + '"); ' +
...
ChatUserActionsClass.prototype.sendTicketReply = function(ticket, receiver, bcc, subject, message, comment, attachments, messageId, previousMessageId, addToWL) {
    this.lzm_chatPollServer.pollServerTicket([{id: ticket.id, ed: this.lzm_chatDisplay.myId, me: message, re: receiver, bcc: bcc, lg: ticket.l, gr: ticket.gr,
