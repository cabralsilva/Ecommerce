...
function EventConfiguration() {
    ...
    this.m_Definitions = [
        ...
        {name:'Actions',actions:[
            {name:'InternalMessageBox',target:'operator', editable: true, operatorList:true, title:tidc('ev_action_0'), type:'string', value:''},
            {name:'InternalPlaySound',target:'operator', editable: true, operatorList:true, title:tidc('ev_action_1'), type:'string', value:'',titlebottom: tidc('example') + ' https://www.mysite.domain/sounds/event.mp3'},
            {name:'ExternalInvitation',target:'visitor', editable: true, custom:true,title:tidc('ev_action_2')},
            ...
            {name:'ExternalTag',target:'visitor', editable: true, title:tidc('ev_action_6'), type:'area', value:''}
        ]}
    ];
}
...
EventConfiguration.prototype.showEventConfiguration = function() {
    ...
    lzm_displayHelper.createDialogWindow(headerString, bodyString, footerString, 'event-configuration', {}, {}, {}, {}, '', dialogData, true, false, 'event_configuration_dialog');
...
EventConfiguration.prototype.saveEvent = function() {
    ...
    lzm_chatPollServer.pollServerSpecial(this.getPostDataObject(false), 'event');
    this.m_SelectedEvent = null;
};
...
EventConfiguration.prototype.deleteEvent = function(id) {
    ...
    $('#alert-btn-ok').click(function()
    {
        ...
        lzm_chatPollServer.pollServerSpecial(that.getPostDataObject(true), 'event');
...
EventConfiguration.prototype.getPostDataObject = function(remove) {
    ...
    pObj[p+'vp_0'] = $('#event-priority').val();
    ...
    else
    {
        ...
        counter = 0;
        for(key in lzm_chatServerEvaluation.global_configuration.database['goals'])
        {
            se = lzm_chatServerEvaluation.global_configuration.database['goals'][key];
        ...
        for(key in this.m_SelectedEvent.DataConditions){
            ...
            pObj[p+'vw_0_b_' + counter] = lz_global_base64_decode(se.Value);
            counter++;
        }
        ...
        for(key in this.m_SelectedEvent.Actions){
            ...
            var actionType = parseInt(se.type);
            pObj[p+'vj_0_a_' + counter] = se.id;
            pObj[p+'vj_0_b_' + counter] = se.type;
            pObj[p+'vj_0_c_' + counter] = se.val;

            if(this.m_Definitions[1].actions[actionType].name == 'ExternalInvitation'){
            ...
            else if(this.m_Definitions[1].actions[actionType].name == 'ExternalOverlayBox'){
            ...
            else if(this.m_Definitions[1].actions[actionType].name == 'InternalPlaySound' || this.m_Definitions[1].actions[actionType].name == 'InternalMessageBox'){
            ...
            else if(this.m_Definitions[1].actions[actionType].name == 'ExternalWebsitePush'){
...
EventConfiguration.prototype.showEventCreationForm = function(type,id) {
    ...
    lzm_displayHelper.createDialogWindow(headerString, bodyString, footerString, 'edit-event', {}, {}, {}, {}, '', dialogData, true, false);
    ...
    $('#save-event').click(function() {
        if(lzm_commonPermissions.permissions.events=='1' && that.m_SelectedEvent.creator != '' && that.m_SelectedEvent.creator != lzm_chatServerEvaluation.myId)
...
EventConfiguration.prototype.elementTypeExists = function(type,name){
    ...
    for(var keya in allObjects)
        for(var keyo in allObjects[keya]){
            ...
            {
                if(d(allObjects[keya][keyo].type) && this.m_Definitions[1].actions[parseInt(allObjects[keya][keyo].type)].name == name)
                    return true;
            }
        }
    return false;
...
EventConfiguration.prototype.showEventSubElementCreation = function(action,type,id){
    ...
    elementSelect.change(function(){
        ...
        else
        {
            ...
            var html = (d(def.type)) ? lzm_displayLayout.getControlHTML(def) : '';
...
EventConfiguration.prototype.applyRow = function(action,subType,elementType,obj,objId,def){
    ...
    if(subType=='actions'){
        ...
        if(elementType == 'ExternalInvitation'){
            ...
            subElement.type = '2';
            if(!saveSenders(subElement.Invites[0],'inv',!selGroup) && !selGroup)
                return false;
        }
...
EventConfiguration.prototype.createSubElementCustomHtml = function(definition,objId){
    ...
    if(definition.name == 'URLCondition'){
        ...
        for (var key in lzm_chatServerEvaluation.inputList.objects)
            if(lzm_chatServerEvaluation.inputList.objects[key].active==1)
                inputs.push({text:lzm_chatServerEvaluation.inputList.objects[key].name,value:lzm_chatServerEvaluation.inputList.objects[key].id});
...
EventConfiguration.prototype.applySubElementCustomLogic = function(definition,obj,objId){
    ...
    if(definition.name == 'DataCondition' && obj != null){
        ...
        var groupList = [], groups = lzm_chatServerEvaluation.groups.getGroupList('id',true,false);
...
EventConfiguration.prototype.createEventHtml = function(tab){
    ...
    if(tab==0){
        ...
        for(kg in lzm_chatServerEvaluation.global_configuration.database['goals'])
        {
            goal = lzm_chatServerEvaluation.global_configuration.database['goals'][kg];
            checked = false;
            for(kge in this.m_SelectedEvent.Goals)
                if(this.m_SelectedEvent.Goals[kge].id == goal.id)
                    checked = true;
                thtml += '<tr><td style="width:25px;">'+lzm_inputControls.createCheckbox('cb-event-goal-' + goal.id,'',checked)+'</td><td>'+goal.title+'</td></tr>';
        }

        thtml += '</tbody></table></fieldset></div>';
    }
    return thtml;
};
...
EventConfiguration.prototype.createEventSubActions = function() {
    ...
    for(var key in this.m_SelectedEvent.Actions)
        ahtml += this.createEventSubRowHtml(this.m_SelectedEvent.Actions[key],this.m_Definitions[1].actions[parseInt(this.m_SelectedEvent.Actions[key].type)].name,tid('ev_action_' + this.m_SelectedEvent.Actions[key].type.toString()),'actions',this.m_SelectedEvent.Actions[key].id);
    return ahtml;
};
...
EventConfiguration.prototype.createEventListHtml = function() {
    ...
    for (var i=0; i<lzm_chatServerEvaluation.eventList.length; i++) {
        var ev = lzm_chatServerEvaluation.eventList[i];
        ...
        var onclickAction = (!lzm_chatDisplay.isApp && !lzm_chatDisplay.isMobile) ? ' onclick="selectEventsLine(event, \'' + ev.id + '\');"' : ' onclick="openEventsListContextMenu(event, \'' + ev.id + '\');"';
        var onconetxtMenuAction = (!lzm_chatDisplay.isApp && !lzm_chatDisplay.isMobile) ? ' oncontextmenu="openEventsListContextMenu(event, \'' + ev.id + '\');"' : '';
        var ondblclickAction =  (!lzm_chatDisplay.isApp && !lzm_chatDisplay.isMobile) ? ' ondblclick="showEventCreation(\'edit\', \'' + ev.id + '\', true);"' : '';
...
EventConfiguration.prototype.openEventsListContextMenu = function(e, eventId) {
    ...
    var event = lzm_commonTools.GetElementByProperty(lzm_chatServerEvaluation.eventList,'id',eventId);
    if (event != null) {
        e.stopPropagation();
