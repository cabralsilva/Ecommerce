...
ChatResourcesClass.prototype.setLoading = function(loading){
    if(!loading){
...
ChatResourcesClass.prototype.createQrdTree = function(caller, chatPartner) {
    ...
    var qrdTreeHtml = '<div id="qrd-tree-headline" class="lzm-dialog-headline"></div>' +
        ...
        '<div id="qrd-tree-placeholder"></div>' +
        '</div>' +
    ...
    lzm_displayHelper.createTabControl('qrd-tree-placeholder', [{name: t('All Resources'), content: treeString},
        {name: t('Quick Search'), content: searchString}, {name: t('Recently used'), content: that.CacheRecentlyResources}],
...
ChatResourcesClass.prototype.createButtonLine = function(caller, chatPartner, chatPartnerName){
    var html = '';
    var sendButton = lzm_inputControls.createButton('send-qrd-preview', 'lzm-button-b-active ui-disabled qrd-change-buttons', 'sendQrdPreview(\'\', \'' + chatPartner + '\');',t('To <!--chat-partner-->',[['<!--chat-partner-->',chatPartnerName]]), '', 'lr',{'margin-left': '2px'}, '', 30, '');
    if (caller == 'view-select-panel') {
        ...
        html += lzm_inputControls.createButton('preview-qrd', 'ui-disabled qrd-change-buttons', 'previewQrd(\'' + chatPartner + '\');', tid('preview'), '<i class="fa fa-search"></i>', 'lr', {'margin-left': '3px'},'',30,'');
        ...
        html += lzm_inputControls.createButton('preview-qrd', 'ui-disabled qrd-change-buttons', 'previewQrd(\'' + chatPartner + '\');', '', '<i class="fa fa-search"></i>', 'lr', {'margin-left': '5px'},'',30,'');
        html += lzm_inputControls.createButton('cancel-qrd', '', 'cancelQrd();', t('Cancel'), '', 'lr', {'margin-left': '5px'},'',30,'d');
    }
    return html;
};

ChatResourcesClass.prototype.updateKBInfo = function(count){
    if(this.IsLoading)
...
ChatResourcesClass.prototype.invalidateCache = function(){
    this.CacheResourceList = null;
    this.CachePreparedResources = null;
    this.CacheRecentlyResources = null;
    this.CacheTree = null;
...
ChatResourcesClass.prototype.cacheResources = function(){
    if(this.CacheResourceList == null)
        this.CacheResourceList = lzm_chatServerEvaluation.cannedResources.getResourceList();
...
ChatResourcesClass.prototype.createQrdTreeDialog = function(resources, chatPartner, menuEntry) {
    ...
    that.qrdChatPartner = chatPartner;
    var i;
    var saveResourceSelector = (chatPartner.indexOf('TICKET SAVE') != -1);
    ...
    if (typeof chatPartner == 'undefined' || chatPartner.indexOf('TICKET SAVE') == -1)
        footerString +=  lzm_inputControls.createButton('d-preview-qrd', 'ui-disabled qrd-change-buttons', 'previewQrd(\'' + chatPartner + '\', \'\', true, \'' + menuEntry + '\');','', '<i class="fa fa-search"></i>', 'lr',{'margin-left': '5px'},'',30,'d');
    if (typeof chatPartner != 'undefined' && chatPartner != '') {
        if (chatPartner.indexOf('TICKET LOAD') == -1 && chatPartner.indexOf('TICKET SAVE') == -1 && chatPartner.indexOf('ATTACHMENT') == -1)
            footerString += lzm_inputControls.createButton('d-send-qrd-preview', 'ui-disabled qrd-change-buttons', 'sendQrdPreview(\'\', \'' + chatPartner + '\');',t('To <!--chat-partner-->',[['<!--chat-partner-->',chatPartnerName]]), '', 'lr',{'margin-left': '5px', 'margin-top': '-5px'},'',30,'d');
    ...
    var recentlyString = (!saveResourceSelector) ? that.createQrdRecently(chatPartner, true) : '';
    ...
    while (resources.length > 0 && counter < 10000) {
        ...
        for (i=0; i<resources.length; i++) {
            if (rank == resources[i].ra) {
                ...
                //if(false && $.inArray(resources[i].pid, lzm_chatDisplay.resourcesDisplay.openedResourcesFolder)!=-1)
...
ChatResourcesClass.prototype.fillQrdSearchList = function(chatPartner, inDialog) {
    ...
    var searchString = $('#'+sid+'search-resource').val().replace(/^ */, '').replace(/ *$/, '');
...
ChatResourcesClass.prototype.previewQrd = function(resource, chatPartner, chatPartnerName, chatPartnerUserid, inDialog, menuEntry) {
    ...
    if (typeof chatPartner != 'undefined' && chatPartner != '' && lzm_chatServerEvaluation.userChats.getUserChat(chatPartner) != null &&
        ...
        if (chatPartner.indexOf('TICKET LOAD') == -1 && chatPartner.indexOf('TICKET SAVE') == -1) {
            footerString += '<span style="float:left;">' + lzm_inputControls.createButton('send-preview-qrd', '', 'sendQrdPreview(\'' + resource.rid + '\', \'' + chatPartner + '\');',t('To <!--chat-partner-->',[['<!--chat-partner-->',chatPartnerName]]), '', 'lr',{'margin-left': '8px', 'margin-top': '-5px'},'',30,'d') + '</span>';
...
ChatResourcesClass.prototype.editQrd = function(resource, ticketId, inDialog) {
    ...
    var qrdEditFormString = '<div id="edit-resource" class="lzm-fieldset" data-role="none"' +
        ...
        '<div id="qrd-edit-title-div" class="top-space-half qrd-edit-resource qrd-edit-html-resource qrd-edit-folder-resource qrd-edit-link-resource" style="margin-top: 0;">' +
            lzm_inputControls.createInput('qrd-edit-title', 'resource-input-new', resource.ti, tidc('title'), '', 'text', '') +
        '</div>' +

        '<div class="qrd-edit-resource qrd-edit-html-resource top-space" id="qrd-edit-text-div"><label for="qrd-edit-text">' + tidc('text') + '</label>' +
    ...
    qrdEditFormString += '<div class="qrd-edit-resource qrd-edit-link-resource top-space" id="qrd-edit-url-div">' +
        lzm_inputControls.createInput('qrd-edit-url', 'resource-input-url-new', resource.text, tidc('url'), '', 'text', '') +
        '</div>' +
        '</div></div>';
    ...
    {
        var dialogId = lzm_displayHelper.createDialogWindow(headerString, qrdEditFormString, footerString, 'qrd-edit', defaultCss, {}, {}, {}, '', dialogData, true, false);
...
ChatResourcesClass.prototype.showQrdSettings = function(resource, editorText, caller) {
    ...
    if(resource.ty == 0){
        ...
        knbContent += '<label for="qrd-knb-owner-gr-id-fs">' + tid('group2') + '</label>' + lzm_inputControls.createSelect("qrd-knb-owner-gr-id-fs", groupDisabeld, '', '', {position: 'right', gap: '0px'}, {}, tid('group2'), ownerGRArray, resource.g, '');
    }
    knbContent += '</fieldset>';
...
ChatResourcesClass.prototype.addQrd = function(type, resource, ticketId, inDialog, toAttachment, sendToChat, menuEntry) {
    ...
    var that = this;
    var headerString = (sendToChat == null) ? t('Add new Resource') : (sendToChat.type == 'link') ? t('Send Url to <!--cp_name>', [['<!--cp_name>', sendToChat.cp_name]]) : t('Send File to <!--cp_name>', [['<!--cp_name>', sendToChat.cp_name]]);
    var footerString = '';
    ...
    qrdAddFormString += '<div' + isVisible + ' id="qrd-add-title-div" class="top-space-half qrd-add-resource qrd-add-html-resource qrd-add-folder-resource qrd-add-link-resource">' +
        lzm_inputControls.createInput('qrd-add-title', 'resource-input-new', '', tidc('title'), '', 'text', '') + '</div>';
    ...
    qrdAddFormString += '<div' + isVisible + ' id="qrd-add-url-div" class="top-space qrd-add-link-resource qrd-add-resource">' +
        lzm_inputControls.createInput('qrd-add-url', 'resource-input-url-new', '', tidc('url'), '', 'text', '') + '</div>';
    ...
    qrdAddFormString += '<div' + isVisible + ' id="qrd-add-file-div" class="qrd-add-file-resource qrd-add-resource">' +
        lzm_inputControls.createInput('file-upload-input', 'resource-input-new', '', tidc('file'), '', 'file', '') +
        '<div id="file-upload-progress" style="display: none;"><span id="file-upload-numeric">0%</span><div class="lz_anim_loading"></div></div>' +
        '<div id="file-upload-name" style="margin: 5px 0 2px 0; padding: 2px 4px;"></div>' +
        '<div id="file-upload-size" style="margin: 2px 0; padding: 2px 4px;"></div>' +
        '<div id="file-upload-type" style="margin: 2px 0; padding: 2px 4px;"></div>' +
        '<div id="file-upload-error" style="color: #cc0000; font-weight: bold; padding: 10px 0;"></div>' +
        '<div id="cancel-file-upload-div" style="display: none;">' + lzm_inputControls.createButton('cancel-file-upload','', 'cancelFileUpload()', t('Cancel file upload'), '', 'lr',{'margin-left': '4px', 'display': 'none'}) + '</div>' +
        '</div>' +
        '</div></div>';
    ...
    if (inDialog)
    {
        ...
        {
            dialogData.menu = menuEntry;
            lzm_displayHelper.createDialogWindow(headerString, qrdAddFormString, footerString, 'ticket-details', {}, {}, {}, {}, '', dialogData, true, false, toAttachment + '_attachment');
        } else
        {
            that.qrdTreeDialog[ticketId] = $('#qrd-tree-dialog-container').detach();
            lzm_displayHelper.createDialogWindow(headerString, qrdAddFormString, footerString, 'qrd-tree-dialog', {}, {}, {}, {}, '', dialogData, true, false);
        }
    }
    else
    {
        var showFullscreen = false;//(sendToChat == null) ? true : false;
        var dialogId = (sendToChat == null) ? '' : sendToChat.dialog_id;
        if (sendToChat != null)
            dialogData.menu = headerString;
        dialogId = lzm_displayHelper.createDialogWindow(headerString, qrdAddFormString, footerString, 'qrd-add', {}, {}, {}, {}, '', dialogData, true, showFullscreen, dialogId);
...
ChatResourcesClass.prototype.getResourceIcon = function(resource,showPublic) {
    var that = this;
...
ChatResourcesClass.prototype.getPublicIcon = function(resource) {
    if(resource.p==1)
...
ChatResourcesClass.prototype.createResource = function(resource, chatPartner, inDialog) {
    ...
    if (resource.ty == 0)
    {
        var openMarkIcon = (lzm_chatServerEvaluation.cannedResources.getResourceList('', {parent: resource.rid}).length > 0) ? '<i class="fa fa-caret-right"></i>' : '<i class="fa"></i>';
...
ChatResourcesClass.prototype.createQrdTreeContextMenu = function(myObject)
{
    ...
    if(myObject != 'MENU')
    {
        ...
        contextMenuHtml += '<div' + disabledClass + ' onclick="previewQrd(\'' + myObject.chatPartner + '\');"><i class="fa fa-search"></i><span id="preview-qrd-ctxt" class="cm-line cm-line-icon-left">' + t('Preview') + '</span></div><hr />';
    }


    if(myObject != 'MENU')
    {
        contextMenuHtml += '<div onclick="showSubMenu(\'qrd-tree\', \'kb_add\', \'\', %CONTEXTX%, %CONTEXTY%, %MYWIDTH%, %MYHEIGHT%)"><span class="cm-line cm-click">' + tid('add') + '</span><i class="fa fa-caret-right lzm-ctxt-right-fa"></i></div>';
        disabledClass = (myObject != 'MENU' && (myObject.rid == 1 || myObject.ty == 3 || myObject.ty == 4)) ? ' class="ui-disabled"' : '';
        contextMenuHtml += '<hr><div' + disabledClass + ' onclick="editQrd();"><span id="edit-qrd-ctxt" class="cm-line">' + t('Edit') + '</span></div>';
