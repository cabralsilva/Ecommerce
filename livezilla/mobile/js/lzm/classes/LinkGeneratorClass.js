...
LinkGeneratorClass.prototype.CreateNewCode = function() {
    ...
    var that=this,nameHtml = lzm_inputControls.createInput('template-name','','',t('Name'),'','text','');
    ...
    $('#alert-btn-ok').click(function() {
        ...
        $('#code-list-table tbody').append(that.GetCodeLine(cid,lz_global_base64_encode(JSON.stringify([lz_global_base64_encode(JSON.stringify([])),cid])),that.m_CurrentCodeName));
        that.LoadCode(cid);
...
LinkGeneratorClass.prototype.DeleteCode = function(id){
    ...
    {
        lzm_commonDialog.removeAlertDialog();
        lzm_chatPollServer.pollServerDiscrete('delete_code',{p_cc_c: id});
        $('#cl-'+id).remove();
        that.LoadCode(0);
    });
    $('#alert-btn-cancel').click(function() {
...
LinkGeneratorClass.prototype.CreateLinkGeneratorHtml = function() {
    ...
    var contentHtml =
        ...
        '<table id="link-generator-table" class="hspaced"><tr><td></td><td style="width:45%;">' +
    ...
    contentHtml += '</span></div></div></td>' +
        '<td><fieldset class="lzm-fieldset-full" id="elements-list-div"><legend>'+tid('elements')+'</legend><table class="alternating-rows-table" id="elements-list-table">' +
    ...
    contentHtml += '</span></div></div></div></td><td></td></tr></table>';
...
LinkGeneratorClass.prototype.AddLinkGeneratorElement = function(elementType, editObj) {
    ...
    if(element.m_Type == 'overlay-widget' && parseInt(lzm_commonTools.GetPositionIndex(lzm_displayLayout.getSettingsProperty(element.m_Settings,'m_Position').value)) < 20)
        lzm_displayLayout.getSettingsProperty(element.m_Settings,'m_Position').value = 'right bottom';
    if(element.m_Type == 'inlay-text')
        lzm_displayLayout.getSettingsProperty(element.m_Settings,'m_TextDefault').value = false;

    var tabs = [];
    element.m_Settings.forEach(function(entry_name) {
        if($.inArray(elementType,entry_name.not) === -1)
            tabs.push({name: entry_name.title, content: lzm_displayLayout.getForm(entry_name.name, element, 'lg-element'), hash: entry_name.name});
    });
...
LinkGeneratorClass.prototype.LoadCodesFromServer = function(){
    ...
    lzm_chatPollServer.pollServerDiscrete('get_code_list',null).done(function(data) {
        ...
        $(xmlDoc).find('code').each(function(){
            ...
            clhtml += that.GetCodeLine(cid,lz_global_base64_encode(JSON.stringify([$(this).text(),$(this).attr('i')])),cname);
...
LinkGeneratorClass.prototype.GetCodeLine = function(id,objB64,name){
    return '<tr id="cl-'+id+'" class="lg-cl-code" onClick="loadCode(\''+id+'\');" data-name="'+lz_global_base64_encode(name)+'" data-obj="'+objB64+'"><td style="width:20px;padding:4px;" class="text-center"><i class="fa fa-code icon-large"></i></td><td>'+name+'</td><td style="text-align: right;">' + lzm_inputControls.createButton('delete-element-from-server-btn-' + id, '', 'deleteLinkGeneratorCode(\''+id+'\')', '', '<i class="fa fa-trash" style="color:#808080 !important;"></i>', 'lr',{'margin-right': '0', 'padding-left': '8px', 'padding-right': '8px'}, '', 20, 'b') + '</td></tr>'
};
...
LinkGeneratorClass.prototype.LoadCode = function(id){
    ...
    if(this.m_CurrentCodeId != '' && this.m_CurrentCodeId != id && $('#cl-'+this.m_CurrentCodeId).length){
        ...
        var dobj = lz_global_base64_encode(JSON.stringify([this.GetElementsFromRows(true),this.m_CurrentCodeId]));
        var newRow = this.GetCodeLine(this.m_CurrentCodeId,dobj,oname);
    ...
    else if(this.m_CurrentCodeId != id)
    {
        ...
        var obj = JSON.parse(lz_global_base64_decode($('#cl-'+id).data('obj')));
        var valueObj = obj[0];
        ...
        for (var item in objectList){
            ...
            this.AddElementRow(element.GetRow());
        }
...
LinkGeneratorClass.prototype.GetCode = function(){
    ...
    if(this.isTypeSelected('inlay-text')){
        ...
        code += '<script type="text/javascript" id="'+guid+'" src="<!--server_image--><!--values-->&sid='+guid+'"></script>';
...
function LinkGeneratorElement(_type,_cloneObj) {
    ...
    this.m_Icons['overlay-widget']={icon:'fa fa-square-o',iconcss:'margin-left:4px;'};
    ...
    this.m_Settings = [
        ...
        {name:'Services', title: t('Services'), groups: [
            {name: 'Live Chats', title: t('Live Chats'), controls: [
                ...
                {name:'m_LiveChatsGroup', type:'radio', group: 'live-chat-select', value: false,top:'half', title: t('Public Conversation among all participants of the group:'), left: 'single', not:['overlay-widget']},
                {name:'m_LiveChatsGroupId', type:'array', value:'no_dyn_group', options: dynGroupArray, left: 'double',bottom:'single',top:'half', not:['overlay-widget']}
            ]},
            ...
            {name: 'Phone', title: t('Phone'), controls: [
                ...
                {name:'m_PhoneInbound', type:'bool', value: false, top:'single', title: t('Phone Inbound (Hotline)'), not:['monitoring','overlay-widget','inlay-image','inlay-text','overlay-button-v1']},
        ...
        {name:'Eye Catcher', title: t('Eye-Catcher'), groups: [
            {name: 'EyeCatcher', title: t('Eye-Catcher'), controls: [
                ...
                {name:'m_ECTypeLabel', class:'m_ECUse', type:'label', left:'single', title: tid('type'), top:'single'},
                ...
                if($.inArray(that.m_Type,entry_group.not) === -1){
                    entry_group.controls.forEach(function(entry_control) {
                        if($.inArray(that.m_Type,entry_control.not) === -1){
                            var val = lzm_displayLayout.GetControlValue(entry_control);
                            if(val != null)
...
LinkGeneratorElement.prototype.GetRow = function() {
    return '<tr id="'+this.GetLineId()+'" ondblclick="editLinkGeneratorElement();" onclick="selectLinkGeneratorElement(\'' + this.m_Id + '\');" class="element-list-line lzm-unselectable" data-element="'+lz_global_base64_encode(JSON.stringify(this))+'"><td style="width:20px;padding:4px;" class="text-center"><i class="'+this.m_Icons[this.m_Type].icon+' icon-large"></i></td><td><bs>'+tid(this.m_Type)+'</bs></td></tr>';
};

LinkGeneratorElement.prototype.GetProperty = function(elem){
    return lzm_displayLayout.getSettingsProperty(this.m_Settings,elem);
}
