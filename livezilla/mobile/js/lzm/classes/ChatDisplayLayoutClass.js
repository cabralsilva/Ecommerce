...
ChatDisplayLayoutClass.prototype.resizeAddResources = function() {
    if ($('#qrd-add-body').length > 0 || $('#qrd-tree-dialog-body').length > 0 || $('#ticket-details-body').length > 0) {
        ...
        var thisQrdTextInnerCss = {width:'100%', height:  (qrdTextHeight +30)+'px', border: '1px solid #ccc','background-color': '#f5f5f5'};
        var thisQrdTextInputCss = {height: (qrdTextHeight +30)+'px','box-shadow': 'none', 'border-radius': '0px', padding: '0px', margin: '0px', border: '1px solid #ccc'};
        var thisTextInputBodyCss = {height: (qrdTextHeight - 1)+'px','box-shadow': 'none', 'border-radius': '0px', padding: '0px', margin: '0px','background-color': '#ffffff', 'overflow-y': 'hidden', 'border-top': '1px solid #ccc'};
...
ChatDisplayLayoutClass.prototype.resizeVisitorDetails = function() {
    ...
    $('.dialog-visitor-info, .embedded-visitor-info').each(function(){
        ...
        //$('#visitor-cobrowse-'+elementId+'-action-select').css({'min-width': '0px', width: (visBodyWidth - 40)+'px'});
        ...
        var languageSelectLabelWidth = $('#visitor-cobrowse-'+elementId+'-language-label').width();
        //$('#visitor-cobrowse-'+elementId+'-language-select').css({'min-width': '0px', width: (visBodyWidth - 50 - languageSelectLabelWidth)+'px'});
...
ChatDisplayLayoutClass.prototype.resizeOperatorForwardSelection = function() {
    if ($('#operator-forward-selection-body').length > 0) {
        var fwdTextHeight = Math.max((lzm_chatDisplay.dialogWindowHeight - 227), 100);
        $('#forward-receiver').css({height: (fwdTextHeight-2) + 'px'});
    }
};
...
ChatDisplayLayoutClass.prototype.resizeArchivedChat = function(elementId) {
...
ChatDisplayLayoutClass.prototype.resizeLinkGenerator = function() {
    if ($('#link-generator').length > 0) {
        ...
        $('#code-list-div').css({'min-height': myHeight + 'px'});
...
ChatDisplayLayoutClass.prototype.resizeMenuPanels = function() {
    ...
    $('.main-menu-panel-opt-tools').css({display: (windowWidth>550) ? 'inline-block' : 'none'});
...
ChatDisplayLayoutClass.prototype.resizeMychats = function() {
    if (lzm_chatDisplay.selected_view == 'mychats')
    {
        ...
        $('#chat-info-body').css({left:(ccw-chatInfoWidth)+'px'});
        if($('#chat-info-body').data('hidden')=='1' || ccw < lzm_chatDisplay.minWidthChatVisitorInfo)
        {
...
ChatDisplayLayoutClass.prototype.resizeTicketList = function() {
    if (lzm_chatDisplay.selected_view == 'tickets') {
        ...
        var showTree = (lzm_chatDisplay.windowWidth > 800) && lzm_commonStorage.loadValue('show_ticket_tree_' + lzm_chatServerEvaluation.myId) != 0;
...
ChatDisplayLayoutClass.prototype.resizeObjTranslator = function() {
...
ChatDisplayLayoutClass.prototype.getForm = function(formType,obj,name) {
    var contentHtml = '';
    var that = this;
    ...
    obj.m_Settings.forEach(function(entry_name) {
        if(entry_name.name==formType){
            ...
            {
                if($.inArray(obj.m_Type,entry_group.not) === -1){
                    ...
                    entry_group.controls.forEach(function(entry_control) {
                        if($.inArray(obj.m_Type,entry_control.not) === -1)
...
ChatDisplayLayoutClass.prototype.GetControlValue = function(controlDefinition) {
    if(controlDefinition.type == 'bool' || controlDefinition.type == 'radio')
        return $('#'+lzm_displayLayout.getControlID(controlDefinition)).prop('checked');
        ...
        return $('#'+lzm_displayLayout.getControlID(controlDefinition)).val();
    else if(controlDefinition.type == 'position')
        return ($(".lzm-position-selected",'#'+lzm_displayLayout.getControlID(controlDefinition)).prop('id').replace(lzm_displayLayout.getControlID(controlDefinition), ''));
    return null;
}

ChatDisplayLayoutClass.prototype.getSettingsProperty = function(settings,propertyName) {
    var prop = null;
...
ChatDisplayLayoutClass.prototype.getControlID = function(controlDefintion) {
...
ChatDisplayLayoutClass.prototype.getControlHTML = function(controlDefintion) {
    var contentHtml = '';
