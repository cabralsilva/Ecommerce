...
class OverlayChat
{
    ...
    function GetWaitingMessage()
    {
        ...
        if(!empty(Server::$Configuration->File["gl_wmes"]) && Server::$Configuration->File["gl_wmes"]>-1 && isset($_GET["ovltwo"]))
        {
            ...
            $wmsg .= " <a href=\"javascript:lz_chat_require_leave_message();\"><b>" . LocalizationManager::$TranslationStrings["client_leave_message"] . "</b></a>";
            ...
            if($all > 0)
            {
                ...
                while($row = DBManager::FetchArray($result))
                {
                    if($count++ >= 0)
                    {
                        ...
                        $post = $this->GetPostHTML($postobj->Text,$postobj->Translation,($this->Flags["LPP"] != $postobj->Sender || $pstrchngreq),$postobj->Sender != $USER->Browsers[0]->SystemId,(($postobj->Sender != $USER->Browsers[0]->SystemId) ? $postobj->SenderName : htmlentities($USER->VisitorData->Fullname,ENT_QUOTES,"UTF-8")),$postobj->Created,$postobj->Sender);
