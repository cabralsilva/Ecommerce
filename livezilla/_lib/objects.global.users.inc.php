...
class Visitor extends BaseUser
{
    ...
    function GetRecentXML($_full=false,$xml="")
    {
        if($_full)
        {
            ...
            foreach($this->Browsers as $browser)
                $xml .= $browser->GetXML($this,null,true);
            return $xml . "</rv>";
        }
        else
        {
    ...
    {
        $result = DBManager::Execute(true,"SELECT `t2`.* FROM `".DB_PREFIX.DATABASE_VISITORS."` AS `t1` INNER JOIN `".DB_PREFIX.DATABASE_USER_DATA."` AS `t2` ON `t1`.`data_id`=`t2`.`id` WHERE `t1`.`id`='".DBManager::RealEscape($this->UserId)."';");
        if($result)
            while($row = DBManager::FetchArray($result))
            {
                $this->VisitorData = new UserData();
                $this->VisitorData->SetDetails($row);
                return !$this->VisitorData->IsEmpty();
            }
        return false;
    }
    ...
    {
        //DBManager::Execute(true,"UPDATE `".DB_PREFIX.DATABASE_OPERATORS."` SET `visitor_file_sizes` = REPLACE(`visitor_file_sizes`, '".DBManager::RealEscape($this->UserId)."', '".DBManager::RealEscape(strrev($this->UserId))."');");
...
class VisitorBrowser
{
	...
	{
        $nkey = substr(md5(time()),0,2);
		if($this->FirstCall && $res = DBManager::Execute(true,"INSERT IGNORE INTO `".DB_PREFIX.DATABASE_VISITOR_BROWSERS."` (`id`, `visitor_id`, `visit_id`, `created`, `last_active`, `last_update`, `is_chat`,`pre_message`,`overlay`,`overlay_container`) VALUES ('".DBManager::RealEscape($this->BrowserId)."','".DBManager::RealEscape($this->UserId)."','".DBManager::RealEscape($this->VisitId)."','".DBManager::RealEscape(time())."','".DBManager::RealEscape(time())."','".DBManager::RealEscape($nkey)."','".DBManager::RealEscape($this->Type)."','',".($this->Overlay?1:0).",".($this->OverlayContainer?1:0).");"))
        {
            $this->LastUpdate = $nkey;
        }
    ...
    function ReplaceLoginDetails($_user,$_reset=false,$values="",$keys="",$comma="")
    {
        Server::InitDataBlock(array("INPUTS"));
        foreach(Server::$Inputs as $index => $input)
        {
            $data = ($index!=114) ? $input->GetValue($_user) : "";
...
class VisitorChat extends VisitorBrowser
{
	...
	function SetValues($row, $_dataOnly=false)
	{
        ...
        if(!$_dataOnly)
        {
            ...
            $this->ExternalClosed = !empty($row["external_close"]);$this->FirstActive = $row["first_active"];
