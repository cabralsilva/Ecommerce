    ...
    static function UpdateConfiguration($id = 0)
    {
        if(OperatorRequest::IsValidated() && Is::Defined("VALIDATED_FULL_LOGIN") && OperatorRequest::IsAdministrator(true))
        {
            ...
            if(DB_CONNECTION)
            {
                ...
                $toplevel = array("gl_lzid","gl_pr_cr","gl_db_host","gl_db_user","gl_db_ext","gl_db_eng","gl_db_pass","gl_db_name","gl_db_prefix","gl_root","gl_datprov","gl_insu","gl_insp");
                foreach($_POST as $key => $value)
                    if(strpos($key,"p_cfg_g_")===0)
                    {
                        ...
                        if(!in_array($skey,$toplevel)){
                            ...
                            DBManager::Execute(true,"INSERT INTO `".DB_PREFIX.DATABASE_CONFIG."` (`key`,`value`) VALUES ('".DBManager::RealEscape($skey)."','".DBManager::RealEscape($value)."');");
                        }
                    }
            }
