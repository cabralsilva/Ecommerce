...
class ExternalChat
{
    ...
    static function Login($_user,$_group)
    {
        ...
        $_user->VisitorData->LoadFromLogin($_group);
        if(!empty($_POST["p_cmb"]))
        {
            $_user->Browsers[0]->CallMeBack = true;
            $_user->Browsers[0]->SetCallMeBackStatus(1);
        }
        $_user->ApplyVisitorData();
        $_user->VisitorData->SaveToCookie();
        $_user->Browsers[0]->Subject = $_user->VisitorData->Text;
    ...
    static function Listen($_user,$init=false)
    {
        ...
        if(!(IS_FILTERED && !FILTER_ALLOW_CHATS))
        {
                ...
                else if((count(VisitorChat::$Router->OperatorsAvailable) + count(VisitorChat::$Router->OperatorsBusy)) > 0)
                {
                    ...
                    else
                    {
                        ...
                        if(empty($_GET["acid"]))
                        {
                            ...
                            $USER->Browsers[0]->ShowQueueInformation($USER,$chatPosition,$chatWaitingTime,$qtext);
            ...
            if($USER->Browsers[0]->Status <= CHAT_STATUS_WAITING && empty($_POST["p_wls"]) && empty(VisitorChat::$DynamicGroup))
                $USER->AddFunctionCall("lz_chat_show_waiting_links('".base64_encode($wl = Server::$Groups[$USER->Browsers[0]->DesiredChatGroup]->GetWaitingLinks($USER->Browsers[0]->Subject,Visitor::$BrowserLanguage))."');",false);
        }
        else
            displayFiltered();
        return $USER;
