...
class OperatorRequest
{
    ...
    static function Listen()
    {
        ...
        if(SERVERSETUP || LOGIN || Server::$Operators[CALLER_SYSTEM_ID]->LastActive <= Server::$Configuration->File["gl_lcut"])
...
class ExternalXMLBuilder
{
	...
	function Generate()
	{
		...
		if(is_array($this->Visitors))
			...
			{
                array_push($this->ActiveVisitors,$userid);
                ...
                if(!$icw)
				{
                    ...
                    if(Server::$Operators[CALLER_SYSTEM_ID]->AppClient && empty(Server::$Configuration->File["gl_apvm"]))
                        continue;
				}
				...
				if($icw || !(!empty(Server::$Configuration->File["gl_hvjd"]) && empty($USER->Javascript)))
				{
					...
					{
                        $vi = $USER->VisitorData->GetXML();
						if($this->IsDiscardedObject || $isactivebrowser)
							$this->XMLCurrent .= $this->XMLCurrentVisitorTag . $this->XMLCurrentVisitor . $vi . "</v>\r\n";
						$this->XMLVisitorOpen = false;
					}
				}
			}
		$this->RemoveFileSizes($this->ActiveVisitors,$this->ActiveBrowsers);
	}
	...
	function AddBrowserXML()
	{
		...
		if($this->CurrentResponseType != DATA_RESPONSE_TYPE_KEEP_ALIVE)
		{
            if(count($BROWSER->History)>0)
                $this->XMLCurrentVisitor .= $BROWSER->GetXML($USER, $this->XMLCurrentChat,$visitorDetails);
	...
	function BuildChatXML()
	{
		...
		if(!$BROWSER->Closed && ($BROWSER->Status > CHAT_STATUS_OPEN || $BROWSER->Waiting))
		{
			if(!empty($BROWSER->DesiredChatGroup))
			{
				...
				$USER->IsChat = true;
				$this->XMLCurrentChat = "<chat id=\"".base64_encode($BROWSER->ChatId)."\" hc=\"".base64_encode($BROWSER->HistoryCounts)."\" d=\"".base64_encode(!empty($BROWSER->Declined) ? 1 : 0)."\" p=\"".base64_encode($BROWSER->Priority)."\" f=\"".base64_encode($BROWSER->FirstActive)."\" q=\"".base64_encode(($BROWSER->Status > CHAT_STATUS_OPEN) ? "0" : "1")."\" cmb=\"".base64_encode($BROWSER->CallMeBack)."\" st=\"".base64_encode($BROWSER->Activated)."\" fn=\"" . base64_encode($USER->GetInputData(111,true)) . "\" em=\"" . base64_encode($USER->GetInputData(112,true)) . "\" eq=\"" . base64_encode($BROWSER->Subject) . "\" gr=\"".base64_encode($BROWSER->DesiredChatGroup)."\" dcp=\"".base64_encode($BROWSER->DesiredChatPartner)."\" at=\"".base64_encode($BROWSER->AllocatedTime)."\" cp=\"" . base64_encode($USER->GetInputData(116,true))."\" co=\"" . base64_encode($USER->GetInputData(113,true)) . "\"".$pra.$cti.$dgc.">\r\n";
				foreach(Server::$Groups as $groupid => $group)
					if($group->IsDynamic)
						foreach($group->Members as $member => $persistent)
				...
				if(is_array($USER->VisitorData->Customs))
					foreach($USER->VisitorData->Customs as $index => $value)
						if(Server::$Inputs[$index]->Active && Server::$Inputs[$index]->Custom)
                        {
                            $value = (Server::$Inputs[$index]->Type == "Text") ? $USER->GetInputData($index) : $value;
	...
	function RemoveFileSizes($_visitors,$_browsers)
	{
		if(!empty($this->SessionFileSizes))
			foreach($this->SessionFileSizes as $userid => $browsers)
				if(in_array($userid,$_visitors) && is_array($browsers) && count($browsers) > 0)
				{
