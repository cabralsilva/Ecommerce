...
class ImapMailbox {
    ...
    protected function initMailPart(IncomingMail $mail, $partStructure, $partNum)
    {
        ...
        //if(DEBUG_MODE)
          //  Logging::SecurityLog("Debug::Email",@serialize($data),"SYSTEM");

        if($partStructure->encoding == 1)
        {
            //$data = imap_utf8($data);
            // caps issue
            iconv_mime_decode($data,0,"UTF-8");
        }
        ...
        if((empty($params['charset']) || $params['charset'] == "X-UNKNOWN") && function_exists("mb_detect_encoding"))
            $params['charset'] = mb_detect_encoding($data);
        else if($params['charset'] == "X-UNKNOWN")
