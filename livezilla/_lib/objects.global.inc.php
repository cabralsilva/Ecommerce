...
class Server
{
    ...
    {
        if(in_array("VISITOR",$_fields))
        {
            CacheManager::FlushKey(DATA_CACHE_KEY_VISITORS);
    ...
    static function CloseChats()
    {
        ...
        while($row = DBManager::FetchArray($result))
        {
            ...
            if($results && $rows = DBManager::FetchArray($results))
            {
                ...
                Chat::SaveToArchive($row["chat_id"],$visitor->VisitorData->Fullname,$rows["visitor_id"],$lastOp,$rows["request_group"],$visitor->VisitorData->Email,$visitor->VisitorData->Company,$visitor->VisitorData->Phone,$row["host"],$row["ip"],$chat->Subject,(empty(Server::$Configuration->File["gl_sctb"]) && $botchat),$waitingtime,$duration,$startResult,$endResult);
            }
        }
    }
}
    ...
    function MatchDataConditions($_visitor)
    {
        ...
        foreach($this->DataConditions as $dcarr)
        {
            ...
            foreach(Server::$Inputs as $input)
            {
                ...
                {
                    if(Is::WildcardMatch($value,$input->GetValue($_visitor)))
                        return true;
                }
            }
        }
...
class CacheManager
{
    ...
    {
        CacheManager::WriteDataUpdateTime($_areaIndex,false,DBManager::$Connector,DB_PREFIX,round(microtime(true) * 100)."0");
    }
        ...
        foreach(Server::$Inputs as $index => $input)
            if($input->Custom && $input->Active)
            {
                if(!empty($_GET["cf".$index]))
                    ...
                    $this->Customs[$index] = $input->GetServerInput();
            }
    }
    ...
    function GetXML()
    {
        ...
        if(is_array($this->Customs))
            foreach($this->Customs as $i => $c)
                $xml .= "<cf i=\"".base64_encode($i)."\">" . base64_encode($this->GetInputData($i,true)) . "</cf>";
        return $xml . "</d>";
    }
...
class GeoTracking
{
    ...
    {
        $_toReplace = str_replace("<!--geo_url-->",GeoTracking::GetURL() . "?aid=1&sid=".base64_encode(Server::$Configuration->File["gl_lzid"])."&dbp=".Server::$Configuration->File["gl_gtdb"],$_toReplace);
