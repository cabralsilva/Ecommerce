...
if(!empty($_REQUEST["cid"]) && Server::InitDataProvider())
{
    ...
    if($chat != null)
    {
        ...
        else
        {
            ...
            {
                if(StoreFile($visitor,$chat->BrowserId,$chat->DesiredChatPartner,$visitor->VisitorData->Fullname,$chat->ChatId))
...
function StoreFile($_visitor,$_browserId,$_partner,$_fullname,$_chatId)
{
    ...
    if($request->Permission == PERMISSION_FULL)
    {
        if(move_uploaded_file($_FILES["form_userfile"]["tmp_name"], PATH_UPLOADS . $request->FileMask))
        {
            ...
            KnowledgeBase::Process($_partner, $_visitor->UserId, $_fullname, 0, $_fullname, 0, 5);
            KnowledgeBase::Process($_partner, $fileid, $filemask, 4, $_FILES["form_userfile"]["name"], 0, $_visitor->UserId, $_FILES["form_userfile"]["size"]);
