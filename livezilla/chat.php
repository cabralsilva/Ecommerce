...
if(!isset($_GET[GET_EXTERN_TEMPLATE]))
{
    ...
    $html = str_replace("<!--is_public_chat-->",To::BoolString(isset($_GET[GET_EXTERN_DYNAMIC_GROUP])),$html);
    ...
    $html = str_replace("<!--kb_only-->",To::BoolString(!empty($_GET["kbo"])),$html);
    ...
    $html = str_replace("<!--kb_int-->",Server::$Configuration->File["gl_kbin"],$html);

    if(Server::$Configuration->File["gl_kbin"]==2)
        $html = str_replace("<!--kb_curl-->",base64_encode(LocalizationManager::Replace(Server::$Configuration->File["gl_kurl"])),$html);
    else
        $html = str_replace("<!--kb_curl-->",base64_encode(KnowledgeBase::GetExternalURL("",true)),$html);

    $html = str_replace("<!--kb_eurl-->",KnowledgeBase::GetExternalURL("",false),$html);
    ...
    $html = str_replace("<!--template_message_system-->",base64_encode(str_replace("<!--dir-->",LocalizationManager::$Direction,IOStruct::GetFile(TEMPLATE_HTML_MESSAGE_SYSTEM))),$html);
    ...
    $html = str_replace("<!--preselect_ticket-->",To::BoolString(Communication::ReadParameter("t","")=="ticket" || isset($_GET["pt"])),$html);
    $html = str_replace("<!--preselect_kb-->",To::BoolString(Communication::ReadParameter("t","")=="kb" && !isset($_GET["hfk"])),$html);
    $html = str_replace("<!--preselect_callback-->",To::BoolString(Communication::ReadParameter("t","")=="callback"),$html);
    ...
    $html = str_replace("<!--lang_param-->",(isset($_GET[GET_EXTERN_USER_LANGUAGE])) ? "&el=" . Communication::GetParameter(GET_EXTERN_USER_LANGUAGE,"",$nu,null,array(),0,false,false) : "",$html);
}
else
{
	if($_GET[GET_EXTERN_TEMPLATE] == "lz_chat_frame_lgin")
	{
		...
		$html = DataInput::GetChatLoginInputs($html,MAX_INPUT_LENGTH);
        ...
        $html = str_replace("<!--color-->",Colors::TransformBrightness($color,-0.3),$html);
        if(isset($_GET["kbo"]) && !empty(Server::$Configuration->File["gl_knbh"]))
        {
            $kbheader = IOStruct::GetFile(PATH_TEMPLATES."kb_header.tpl");
            $kbheader = ExternalChat::ReplaceLogo($kbheader);
            $html = str_replace("<!--kb_header-->",$kbheader,$html);
        }
    }
	else if($_GET[GET_EXTERN_TEMPLATE] == "lz_chat_frame_chat")
	{
        ...
        if(strlen(Server::$Configuration->File["gl_otrs"])>1)
        {
            ...
            $tlanguages = GroupBuilder::GetLanguageSelects(LocalizationManager::GetBrowserLocalization());
        }
...
$html = str_replace("<!--header-->",ExternalChat::ReplaceLogo(IOStruct::GetFile(PATH_TEMPLATES."header.tpl")),$html);
...
$html = str_replace("<!--url_get_params-->",ExternalChat::GetAllowedParameters(),$html);
