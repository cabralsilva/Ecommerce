...
if(isset($_POST[POST_EXTERN_SERVER_ACTION]))
{
    ...
    if(!empty($error))
    {
        ...
        if(IS_FILTERED && !FILTER_ALLOW_CHATS)
        {
            ...
            $externalUser->AddFunctionCall("lz_chat_add_system_text(2,'".base64_encode("")."');lz_chat_stop_system();",false);
        }
    }
    else
    {
        ...
        else
        {
            if($externalUser->Browsers[0]->Status != CHAT_STATUS_OPEN || $externalUser->Browsers[0]->Waiting)
            {
                ...
                $externalUser->Browsers[0] = new VisitorChat($externalUser->UserId,Encoding::Base64UrlDecode(@$_POST[POST_EXTERN_USER_BROWSERID]),$externalUser->Browsers[0]->DesiredChatGroup,$externalUser->Browsers[0]->DesiredChatPartner);
            }
            else
            {
            ...
            if($_POST[POST_EXTERN_SERVER_ACTION] == EXTERN_ACTION_RELOAD_GROUPS)
            {
                ...
                {
                    if(!$externalUser->LoadVisitorData())
                        $externalUser->VisitorData->LoadFromCookie();
