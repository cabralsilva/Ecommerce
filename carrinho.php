<?php
	...
	//SEO
	$_tituloDescricao = "Carrinho de compras";
...
<div class="carrinho">
  ...
  <div class="conteudo">
    <!-- abre .limites -->
    <div class="limites lista_carrinho">
    </div>
    <!-- fecha .limites -->
...
<script src="<?php echo URL_SSL; ?>js/heartcode-canvasloader-min-0.9.1.js"></script>

<script>
	function carregarLista(acaoPrm,codigoPrm,quantPrm,cepIniPrm,cepFimPrm,tipoFretePrm){
		var data = {
					"codigo_produto_grade":"",
					"quantidade":"",
					"cep_ini":"",
					"cep_fim":"",
					"tipo_frete":"",
		...
		if (codigoPrm!=null)
			data["codigo_produto_grade"] = codigoPrm;
		
		if (quantPrm!=null)
			data["quantidade"] = quantPrm;
		
		if (cepIniPrm!=null)
			data["cep_ini"] = cepIniPrm;
			
		if (cepFimPrm!=null)
			data["cep_fim"] = cepFimPrm;
		
		if (tipoFretePrm!=null)
			data["tipo_frete"] = tipoFretePrm;
		
		$(".lista_carrinho").prepend("<div id=\"canvasloader-container\" class=\"wrapper\"></div>");
		
		insereLoading();
		
		$.ajax({
				type:"POST",
				url: "<?php echo URL_SSL; ?>includes/listagemCarrinho.php"+get,
				data: data,
				...
				success: function(myHtml){
					...
					$(".lista_carrinho").html(myHtml);
				}
		});
		
		//$(".lista_carrinho").load("includes/listagemCarrinho.php"+get,data);
	}
	/*
	function alterarQuantidade(){
		...
		try {
			...
			carregarLista("quantidade",idProd,(quant)+"");
		}catch(err) {
			carregarLista("quantidade",idProd,"1");
		}
	}
	*/
	function insereLoading(){
		...
		$("#canvasloader-container").css("position","absolute");
	    $("#canvasloader-container").css("margin-top", "4px");
	    $("#canvasloader-container").css("margin-left", "360px");
